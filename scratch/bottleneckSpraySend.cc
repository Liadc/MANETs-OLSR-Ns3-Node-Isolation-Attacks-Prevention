...
void findDisjoinedPathsAndSend(NodeContainer *nodes){
	std::set<Ipv4Address> senders = nodes->Get(1)->GetObject<RoutingProtocol>()->findDisjoinedPaths(Ipv4Address("10.0.0.2"), Ipv4Address("10.0.0.1"));
	ApplicationContainer apps;
	for (size_t i=0; i< nodes->GetN(); ++i){
		if (senders.count(nodes->Get(i)->GetObject<Ipv4>()->GetAddress(1,0).GetLocal())){
			...
			udpClientHelper.SetAttribute("PacketSize", UintegerValue(1000+i));

			apps.Add(udpClientHelper.Install(nodes->Get(i)));

		}
	}
	apps.Start(Seconds(0));

	std::cout << "Disjoined Paths:\n";
	for (std::set<Ipv4Address>::const_iterator it = senders.begin(); it!=senders.end(); ++it){
		std::cout << *it << "\n";
	}
	std::cout << "\n\n";
}
...
int main (int argc, char *argv[]){
	...
	cmd.AddValue("bFindPaths", "Find disjoined paths and send packets", bFindPaths);
	cmd.Parse (argc, argv);
	...
	if (bFindPaths){
		Simulator::Schedule(Seconds (60), &findDisjoinedPathsAndSend, &nodes);	
	}
