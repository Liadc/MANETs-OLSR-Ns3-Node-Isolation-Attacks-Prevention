...
void MakeBetTwoWay (NodeContainer* cont, Ipv4Address s, Ipv4Address t){
	std::cout << Simulator::Now().GetSeconds() << "s:\n";
	//std::vector<std::pair<double, Ipv4Address> > bets = cont->Get(2)->GetObject<RoutingProtocol>()->FindPotentionalBottleNeck(s, t);
	std::vector<std::pair<double, Ipv4Address> > bets = cont->Get(1)->GetObject<RoutingProtocol>()->FindPotentionalBottleNeck(s, t);
	std::vector<std::pair<double, Ipv4Address> > betsr = cont->Get(1)->GetObject<RoutingProtocol>()->FindPotentionalBottleNeck(t, s);
	std::vector<std::pair<double, Ipv4Address> > result;
	if (bets.empty() || bets.size() == 0) {
		std::cout << "No prediction\n\n";
		return;
	}
	if (betsr.empty() || betsr.size() == 0) {
		std::cout << "No prediction\n\n";
		return;
	}
	for (size_t i = 0; i < bets.size(); ++i){
		for (size_t j = 0; j < betsr.size(); ++j){
			if (bets[i].second == betsr[j].second){
				result.push_back(std::make_pair(bets[i].first * betsr[j].first, bets[i].second));
				break;
			}
		}
	}
...
int main (int argc, char *argv[]){
	...
	if (bUdpServer){
		...
		UdpClientHelper udpClientHelper2(Ipv4Address("10.0.0.2"), 80);
		udpClientHelper2.SetAttribute("Interval", TimeValue(Seconds(7)));
		udpClientHelper2.SetAttribute("MaxPackets", UintegerValue(100000000));
		udpClientHelper2.SetAttribute("PacketSize", UintegerValue(1025));

		apps.Add(udpServerHelper.Install(nodes.Get(1)));
		...
		apps.Add(udpClientHelper2.Install(nodes.Get(0)));
	...
	if (bMakeBets){
		for (size_t i=60; i<nSimulationSeconds; i+=7){
			Simulator::Schedule(Seconds (i), &MakeBetTwoWay, &nodes, Ipv4Address("10.0.0.2"), Ipv4Address("10.0.0.1"));
		}
	}
