...
int main (int argc, char *argv[]){
	...
	// Variables
	uint32_t nNodes = 10; //Number of nodes in the simulation, default 100
	...
	bool bMobility = true; //Delcares whenever there is movement in the network
	...
	bIsolationAttackNeighbor = false; //Execute isolation attack by a random neighbor
	...
	uint32_t startAttackTime = 8; //The time for an attacker to start attacking.
	uint32_t startDefenceTime = 162; //The time for defense to start.
	uint32_t startUdpSend = 200; //The time to start sending messages to victim. total 18 msgs. 4sec intervals. 72seconds to complete.
	...
	if (bUdpServer){
		...
		udpClientHelper.SetAttribute("Interval", TimeValue(Seconds(4))); //The time to wait between packets
