...
void IsSendingThroughMPR(Ptr<Node> node, Ipv4Address addr){
	//for (size_t i=0; i<nodes->GetN();++i){
	std::cout << "Passing message through MPR - " << node->GetObject<RoutingProtocol>()->sendThroughMpr(addr) << std::endl;
	//}
}
...
int main (int argc, char *argv[]){
	...
	CommandLine cmd;
	cmd.AddValue("nNodes", "Number of nodes in the simulation - For each part of the network ", nNodes);
	cmd.AddValue("nMaxGridX", "X of the simulation rectangle for each part", nMaxGridX);
	cmd.AddValue("nMaxGridY", "Y of the simulation rectangle for each part", nMaxGridY);
	...
	cmd.AddValue("bPrintNeighbors", "Print Neighbors messages", bPrintNeighbors);
	cmd.AddValue("bPrintTC", "Print TC messages", bPrintTC);
	cmd.AddValue("bPrintMPR", "Print MPR selected by each node", bPrintMPR);
	...
	// Build network
	NodeContainer nodes;
	NodeContainer nodesSendReceive;
	NodeContainer nodesA;
	NodeContainer nodesB;
	NodeContainer nodesConnection;

	nodesSendReceive.Create (2);
	nodesConnection.Create (4);
	nodesA.Create (nNodes);
	nodesB.Create (nNodes);
	nodes.Add (nodesSendReceive);
	nodes.Add (nodesConnection);
	nodes.Add (nodesA);
	nodes.Add (nodesB);
	...
	double dShiftX = 50 * 2 + 400;
	{
		MobilityHelper mobility;
		Ptr<UniformRandomVariable> randomGridX = CreateObject<UniformRandomVariable> ();
		Ptr<UniformRandomVariable> randomGridY = CreateObject<UniformRandomVariable> ();
		randomGridX->SetAttribute ("Min", DoubleValue (dShiftX + dMaxGridX + 11));
		randomGridX->SetAttribute ("Max", DoubleValue (dShiftX + 2* dMaxGridX - 11));
		randomGridY->SetAttribute ("Min", DoubleValue (11));
		randomGridY->SetAttribute ("Max", DoubleValue (dMaxGridY-11));
		...
		if (bMobility) {
			mobility.SetMobilityModel ("ns3::RandomWalk2dMobilityModel",
					"Bounds", RectangleValue (Rectangle (dShiftX + dMaxGridX + 0, dShiftX + 2*dMaxGridX, 0, dMaxGridY)),
					//"Speed", StringValue("ns3::UniformRandomVariable[Min=0|Max=2.0]"),
					"Speed", StringValue("ns3::UniformRandomVariable[Min=1.5|Max=2.0]"),
					//"Speed", StringValue("ns3::UniformRandomVariable[Min=9.5|Max=10.0]"),
					"Time", TimeValue(Seconds(3.0)),
	...
	}
	{
		...
		Ptr<UniformRandomVariable> randomGridX = CreateObject<UniformRandomVariable> ();
		Ptr<UniformRandomVariable> randomGridY = CreateObject<UniformRandomVariable> ();
		randomGridX->SetAttribute ("Min", DoubleValue (dShiftX + 11));
		randomGridX->SetAttribute ("Max", DoubleValue (dShiftX + dMaxGridX-11));
		randomGridY->SetAttribute ("Min", DoubleValue (11));
		randomGridY->SetAttribute ("Max", DoubleValue (dMaxGridY-11));
		...
		mobility.SetPositionAllocator (taPositionAlloc);
		mobility.SetMobilityModel ("ns3::ConstantPositionMobilityModel");
		mobility.Install (nodesConnection);

		Vector vec;
		vec.x = dMaxGridX + 50;
		vec.y = dMaxGridY / 2;
		nodesConnection.Get(0)->GetObject<MobilityModel>()->SetPosition(vec);		
		vec.x = dShiftX + dMaxGridX - 50;
		nodesConnection.Get(1)->GetObject<MobilityModel>()->SetPosition(vec);		
		vec.x = dShiftX / 2 + dMaxGridX;
		vec.y = dMaxGridY / 2 + 30;
		nodesConnection.Get(2)->GetObject<MobilityModel>()->SetPosition(vec);		
		vec.y = dMaxGridY / 2 - 30;
		nodesConnection.Get(3)->GetObject<MobilityModel>()->SetPosition(vec);		
	}
	
	if (bFixPos){

		mobility.SetMobilityModel ("ns3::ConstantPositionMobilityModel");
		mobility.Install (nodesSendReceive.Get(0));
		mobility.Install (nodesSendReceive.Get(1));
		Vector vec;
		vec.x = 50;
		vec.y = 50;
		nodesSendReceive.Get(0)->GetObject<MobilityModel>()->SetPosition(vec);		
		vec.x = dMaxGridX*2 - 50;
		vec.y = dMaxGridY - 50;
		nodesSendReceive.Get(1)->GetObject<MobilityModel>()->SetPosition(vec);	
//	} else {
//		mobility.Install (nodes);
	...
	//	apps2.Stop(Seconds(nSimulationSeconds));
		...
		Simulator::Schedule(Seconds(60), &IsSendingThroughMPR, nodes.Get(3), Ipv4Address("10.0.0.1"));
	}
