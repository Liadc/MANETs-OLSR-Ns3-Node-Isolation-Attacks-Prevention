...
//		if (pt->sixNodesArrangedInCircle()) ++count;
//	}
//	//NS_LOG_INFO("Total fakes required: " << count << " out of " << cont->GetN());
//	std::cout << "In 6 ring: " << count << "/" << cont->GetN() << std::endl;
}
...
static void PrintCountFakeNodes(NodeContainer* cont){
	...
	for (unsigned int i=0;i<cont->GetN();++i){
		if(i==2 || i == 0) { //attacker and victim should not be counted as defenders who advertise fictives.
			continue;
		}
		Ptr<RoutingProtocol> pt = cont->Get(i)->GetObject<RoutingProtocol>();
		//NS_LOG_INFO("Node id: " << i << "\tTime: " << Simulator::Now().GetSeconds() << "\tRequireFake? " << pt->RequireFake());
		if (pt->RequireFake()){
			++count; 
			//std::cout << "address of node declaring fictive: " << pt->getCurrentAddr() << std::endl;
			std::cout << "Fictive advertised by: " << pt->getCurrentAddr() << std::endl;
		} 
	}
	NS_LOG_INFO("Total fakes required: " << count << " out of " << cont->GetN());
...
*/
	...
	//... NS_LOG_INFO broke. Using cout...
	//std::cout << "Total fraction of MPR: " << sum / (double) cont->GetN() << std::endl;
	std::cout << "Total MPRs: " << sum << std::endl;
...
int main (int argc, char *argv[]){
	...
	bool bPrintFakeCount = true;
	bool bPrint6RingCount = false;
	bool bPrintMprFraction = true;
	bool bPrintRiskyFraction = false;
	bool bIsolationAttack = false;
	bool bIsolationAttackBug = false; //attacker following victim
	bool bIsolationAttackNeighbor = false;
	bool bEnableFictive = true;
	bool bHighRange = false;
	bool bPrintTcPowerLevel = true;
	bool bNeighborDump = false;
	bool bIsolationAttackMassive = false;
	bool bConnectivityPrecentage = true;
	bool bUdpServer = true;
	...
	Vector vec;
	vec.x = dMaxGridX - 50 - 5;
	vec.y = 300;
	nodes.Get(3)->GetObject<MobilityModel>()->SetPosition(vec);		
	vec.x = dMaxGridX - 50 - 300 - 5;
	nodes.Get(0)->GetObject<MobilityModel>()->SetPosition(vec);
	vec.x = dMaxGridX - 100 - 5;
	vec.y = 100;
	nodes.Get(4)->GetObject<MobilityModel>()->SetPosition(vec);
	vec.y = 500;
	nodes.Get(5)->GetObject<MobilityModel>()->SetPosition(vec);
	vec.x = dMaxGridX - 300 - 5;
	vec.y = 100;
