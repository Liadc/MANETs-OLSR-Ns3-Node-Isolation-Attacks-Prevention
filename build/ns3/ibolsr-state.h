...
/// \brief	This header file declares and defines internal state of an IBOLSR node.

#ifndef IBOLSR_STATE_H
#define IBOLSR_STATE_H

#include "ibolsr-repositories.h"

namespace ns3 {

using namespace ibolsr;

/// This class encapsulates all data structures needed for maintaining internal state of an IBOLSR node.
class IBOlsrState
{
  //  friend class IBOlsr;
...
public:
  ...
  {
    std::clog<<"DestAddress      Exp.Time         LastAddress      Seq.Number"<<std::endl;
    for (unsigned int i=0;i<m_topologySet.size();i++)
      std::clog<<m_topologySet[i].destAddr
               <<"      "<<m_topologySet[i].expirationTime
               <<"      "<<m_topologySet[i].lastAddr
               <<"      "<<m_topologySet[i].sequenceNumber
               <<std::endl;
  }   
  //bool RequireFake();
  //double FractionOfMpr();
