...
#include "ns3/test.h"
#include "ns3/lsr-header.h"
#include "ns3/packet.h"

namespace ns3 {

class LsrEmfTestCase : public TestCase {
public:
  LsrEmfTestCase ();
  virtual void DoRun (void);
};

LsrEmfTestCase::LsrEmfTestCase ()
  : TestCase ("Check Emf lsr time conversion")
{
}
void
LsrEmfTestCase::DoRun (void)
{
  for (int time = 1; time <= 30; time++)
    {
      uint8_t emf = lsr::SecondsToEmf (time);
      double seconds = lsr::EmfToSeconds (emf);
...
class LsrMidTestCase : public TestCase {
public:
  LsrMidTestCase ();
  virtual void DoRun (void);
};

LsrMidTestCase::LsrMidTestCase ()
  : TestCase ("Check Mid lsr messages")
{
}
void
LsrMidTestCase::DoRun (void)
{
  Packet packet;

  {
    lsr::PacketHeader hdr;
    lsr::MessageHeader msg1;
    lsr::MessageHeader::Mid &mid1 = msg1.GetMid ();
    lsr::MessageHeader msg2;
    lsr::MessageHeader::Mid &mid2 = msg2.GetMid ();
    ...
    msg2.SetMessageType (lsr::MessageHeader::MID_MESSAGE);
    msg2.SetMessageSequenceNumber (7);

    // Build an LSR packet header
...

  {
    ...
    uint32_t sizeLeft = hdr.GetPacketLength () - hdr.GetSerializedSize ();
    {
      ...
      NS_TEST_ASSERT_MSG_EQ (msg1.GetMessageType (), lsr::MessageHeader::MID_MESSAGE, "204");
    ...
    }
    {
      ...
      NS_TEST_ASSERT_MSG_EQ (msg2.GetMessageType (), lsr::MessageHeader::MID_MESSAGE, "212");
...
class LsrHelloTestCase : public TestCase {
public:
  LsrHelloTestCase ();
  virtual void DoRun (void);
};

LsrHelloTestCase::LsrHelloTestCase ()
  : TestCase ("Check Hello lsr messages")
{
}
void
LsrHelloTestCase::DoRun (void)
{
  Packet packet;
  lsr::MessageHeader msgIn;
  lsr::MessageHeader::Hello &helloIn = msgIn.GetHello ();

  helloIn.SetHTime (Seconds (7));
  helloIn.willingness = 66;

  {
    lsr::MessageHeader::Hello::LinkMessage lm1;
    lm1.linkCode = 2;
    ...
    lsr::MessageHeader::Hello::LinkMessage lm2;
    lm2.linkCode = 3;
  ...
  lsr::MessageHeader msgOut;
  packet.RemoveHeader (msgOut);
  lsr::MessageHeader::Hello &helloOut = msgOut.GetHello ();
...
class LsrTcTestCase : public TestCase {
public:
  LsrTcTestCase ();
  virtual void DoRun (void);
};

LsrTcTestCase::LsrTcTestCase ()
  : TestCase ("Check Tc lsr messages")
{
}
void
LsrTcTestCase::DoRun (void)
{
  Packet packet;
  lsr::MessageHeader msgIn;
  lsr::MessageHeader::Tc &tcIn = msgIn.GetTc ();
  ...
  lsr::MessageHeader msgOut;
  packet.RemoveHeader (msgOut);
  lsr::MessageHeader::Tc &tcOut = msgOut.GetTc ();
...
class LsrHnaTestCase : public TestCase {
public:
  LsrHnaTestCase ();
  virtual void DoRun (void);
};

LsrHnaTestCase::LsrHnaTestCase ()
  : TestCase ("Check Hna lsr messages")
{
}

void
LsrHnaTestCase::DoRun (void)
{
  Packet packet;
  lsr::MessageHeader msgIn;
  lsr::MessageHeader::Hna &hnaIn = msgIn.GetHna ();

  hnaIn.associations.push_back ((lsr::MessageHeader::Hna::Association)
  ...
  lsr::MessageHeader msgOut;
  packet.RemoveHeader (msgOut);
  lsr::MessageHeader::Hna &hnaOut = msgOut.GetHna ();
...
static class LsrTestSuite : public TestSuite
{
public:
  LsrTestSuite ();
} g_lsrTestSuite;

LsrTestSuite::LsrTestSuite()
  : TestSuite ("routing-lsr-header", UNIT)
{
  AddTestCase (new LsrHnaTestCase (), TestCase::QUICK);
  AddTestCase (new LsrTcTestCase (), TestCase::QUICK);
  AddTestCase (new LsrHelloTestCase (), TestCase::QUICK);
  AddTestCase (new LsrMidTestCase (), TestCase::QUICK);
  AddTestCase (new LsrEmfTestCase (), TestCase::QUICK);
}
