...
///
/// \brief Implementation of LSR agent and related classes.
///
/// This is the main file of this software because %LSR's behaviour is
/// implemented here.
///
...
///
/// We only use this value in order to define LSR_NEIGHB_HOLD_TIME.
///
#define LSR_REFRESH_INTERVAL   m_helloInterval


/********** Holding times **********/

/// Neighbor holding time.
#define LSR_NEIGHB_HOLD_TIME   Time (3 * LSR_REFRESH_INTERVAL)
/// Top holding time.
#define LSR_TOP_HOLD_TIME      Time (3 * m_tcInterval)
/// Dup holding time.
#define LSR_DUP_HOLD_TIME      Seconds (30)
/// MID holding time.
#define LSR_MID_HOLD_TIME      Time (3 * m_midInterval)
/// HNA holding time.
#define LSR_HNA_HOLD_TIME      Time (3 * m_hnaInterval)

/********** Link types **********/

/// Unspecified link type.
#define LSR_UNSPEC_LINK        0
/// Asymmetric link type.
#define LSR_ASYM_LINK          1
/// Symmetric link type.
#define LSR_SYM_LINK           2
/// Lost link type.
#define LSR_LOST_LINK          3

/********** Neighbor types **********/

/// Not neighbor type.
#define LSR_NOT_NEIGH          0
/// Symmetric neighbor type.
#define LSR_SYM_NEIGH          1
/// Asymmetric neighbor type.
#define LSR_MPR_NEIGH          2


/********** Willingness **********/

/// Willingness for forwarding packets from other nodes: never.
#define LSR_WILL_NEVER         0
/// Willingness for forwarding packets from other nodes: low.
#define LSR_WILL_LOW           1
/// Willingness for forwarding packets from other nodes: medium.
#define LSR_WILL_DEFAULT       3
/// Willingness for forwarding packets from other nodes: high.
#define LSR_WILL_HIGH          6
/// Willingness for forwarding packets from other nodes: always.
#define LSR_WILL_ALWAYS        7


/********** Miscellaneous constants **********/

/// Maximum allowed jitter.
#define LSR_MAXJITTER          (m_helloInterval.GetSeconds () / 4)
/// Maximum allowed sequence number.
#define LSR_MAX_SEQ_NUM        65535
/// Random number between [0-LSR_MAXJITTER] used to jitter LSR packet transmission.
#define JITTER (Seconds (m_uniformRandomVariable->GetValue (0, LSR_MAXJITTER)))


#define LSR_PORT_NUMBER 698
/// Maximum number of messages per packet.
#define LSR_MAX_MSGS           64

/// Maximum number of hellos per message (4 possible link types * 3 possible nb types).
#define LSR_MAX_HELLOS         12

/// Maximum number of addresses advertised on a message.
#define LSR_MAX_ADDRS          64


namespace ns3 {
namespace lsr {

NS_LOG_COMPONENT_DEFINE ("LsrRoutingProtocol");
...
{
  static TypeId tid = TypeId ("ns3::lsr::RoutingProtocol")
    ...
    .AddAttribute ("Willingness", "Willingness of a node to carry and forward traffic for other nodes.",
                   EnumValue (LSR_WILL_DEFAULT),
                   MakeEnumAccessor (&RoutingProtocol::m_willingness),
                   MakeEnumChecker (LSR_WILL_NEVER, "never",
                                    LSR_WILL_LOW, "low",
                                    LSR_WILL_DEFAULT, "default",
                                    LSR_WILL_HIGH, "high",
                                    LSR_WILL_ALWAYS, "always"))
    .AddTraceSource ("Rx", "Receive LSR packet.",
                     MakeTraceSourceAccessor (&RoutingProtocol::m_rxPacketTrace))
    .AddTraceSource ("Tx", "Send LSR packet.",
                     MakeTraceSourceAccessor (&RoutingProtocol::m_txPacketTrace))
    .AddTraceSource ("RoutingTableChanged", "The LSR routing table has changed.",
...
RoutingProtocol::SetIpv4 (Ptr<Ipv4> ipv4)
{
  NS_ASSERT (ipv4 != 0);
  NS_ASSERT (m_ipv4 == 0);
  NS_LOG_DEBUG ("Created lsr::RoutingProtocol");
  ...
  m_packetSequenceNumber = LSR_MAX_SEQ_NUM;
  m_messageSequenceNumber = LSR_MAX_SEQ_NUM;
  m_ansn = LSR_MAX_SEQ_NUM;
...
void RoutingProtocol::DoInitialize ()
{
  ...
  NS_LOG_DEBUG ("Starting LSR on node " << m_mainAddress);

  Ipv4Address loopback ("127.0.0.1");

  bool canRunLsr = false;
  for (uint32_t i = 0; i < m_ipv4->GetNInterfaces (); i++)
    {
      Ipv4Address addr = m_ipv4->GetAddress (i, 0).GetLocal ();
      if (addr == loopback)
      ...
      InetSocketAddress inetAddr (m_ipv4->GetAddress (i, 0).GetLocal (), LSR_PORT_NUMBER);
      socket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvLsr,  this));
      if (socket->Bind (inetAddr))
        {
          NS_FATAL_ERROR ("Failed to bind() LSR socket");
        }
  ...
  if(canRunLsr)
    {
      ...
      NS_LOG_DEBUG ("LSR on node " << m_mainAddress << " started");
    }
}
...
//
// \brief Processes an incoming %LSR packet following \RFC{3626} specification.
void
RoutingProtocol::RecvLsr (Ptr<Socket> socket)
{
  ...
  NS_LOG_DEBUG ("LSR node " << m_mainAddress << " received a LSR packet from "
                             << senderIfaceAddr << " to " << receiverIfaceAddr);

  // All routing messages are sent from and to port RT_PORT,
  // so we check it.
  NS_ASSERT (inetSourceAddr.GetPort () == LSR_PORT_NUMBER);

  Ptr<Packet> packet = receivedPacket;

  lsr::PacketHeader lsrPacketHeader;
  packet->RemoveHeader (lsrPacketHeader);
  NS_ASSERT (lsrPacketHeader.GetPacketLength () >= lsrPacketHeader.GetSerializedSize ());
  uint32_t sizeLeft = lsrPacketHeader.GetPacketLength () - lsrPacketHeader.GetSerializedSize ();
  ...
  while (sizeLeft)
    {
      ...
      NS_LOG_DEBUG ("Lsr Msg received with type "
  ...
  m_rxPacketTrace (lsrPacketHeader, messages);
...
//         }
      ...
      if (duplicated == NULL)
        {
          switch (messageHeader.GetMessageType ())
            {
            case lsr::MessageHeader::HELLO_MESSAGE:
              NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                            << "s LSR node " << m_mainAddress
            ...
            case lsr::MessageHeader::TC_MESSAGE:
              NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                            << "s LSR node " << m_mainAddress
        ...
        {
          NS_LOG_DEBUG ("LSR message is duplicated, not reading it.");
      ...
      if (do_forwarding)
        {
          ...
          if (messageHeader.GetMessageType ()  != lsr::MessageHeader::HELLO_MESSAGE)
            {
  ...
  // After processing all LSR messages, we must recompute the routing table
      ...
      for (NeighborSet::const_iterator neigh = N.begin ();
           neigh != N.end (); neigh++)
        {
          if (neigh->neighborMainAddr == twoHopNeigh->neighborMainAddr)
            {
              if (neigh->willingness == LSR_WILL_NEVER)
                {
                  ok = false;
                  break;
                }
      ...
      for (NeighborSet::const_iterator neighbor = neighborSet.begin ();
           neighbor != neighborSet.end (); neighbor++)
        {
          if (neighbor->neighborMainAddr == nb2hop_tuple.neighborMainAddr
              && neighbor->willingness != LSR_WILL_NEVER)
            {
              nb2hopOk = true;
              break;
            }
...
///
/// \param msg the %LSR message which contains the HELLO message.
/// \param receiver_iface the address of the interface where the message was received from.
/// \param sender_iface the address of the interface where the message was sent from.
///
void
RoutingProtocol::ProcessHello (const lsr::MessageHeader &msg,
                               const Ipv4Address &receiverIface,
                               const Ipv4Address &senderIface)
{
  NS_LOG_FUNCTION (msg << receiverIface << senderIface);

  const lsr::MessageHeader::Hello &hello = msg.GetHello ();
...
#ifdef NS3_LOG_ENABLE
  {
    const TwoHopNeighborSet &twoHopNeighbors = m_state.GetTwoHopNeighbors ();
    NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                  << "s ** BEGIN dump TwoHopNeighbor Set for LSR Node " << m_mainAddress);
    for (TwoHopNeighborSet::const_iterator tuple = twoHopNeighbors.begin ();
         tuple != twoHopNeighbors.end (); tuple++)
      {
        NS_LOG_DEBUG (*tuple);
      }
    NS_LOG_DEBUG ("** END dump TwoHopNeighbor Set for LSR Node " << m_mainAddress);
  }
#endif // NS3_LOG_ENABLE
...
///
/// \param msg the %LSR message which contains the TC message.
/// \param sender_iface the address of the interface where the message was sent from.
///
void
RoutingProtocol::ProcessTc (const lsr::MessageHeader &msg,
                            const Ipv4Address &senderIface)
{
  const lsr::MessageHeader::Tc &tc = msg.GetTc ();
...
#ifdef NS3_LOG_ENABLE
  {
    const TopologySet &topology = m_state.GetTopologySet ();
    NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                  << "s ** BEGIN dump TopologySet for LSR Node " << m_mainAddress);
    for (TopologySet::const_iterator tuple = topology.begin ();
         tuple != topology.end (); tuple++)
      {
        NS_LOG_DEBUG (*tuple);
      }
    NS_LOG_DEBUG ("** END dump TopologySet Set for LSR Node " << m_mainAddress);
  }
#endif // NS3_LOG_ENABLE
}
...
///
/// \param msg the %LSR message which contains the MID message.
/// \param sender_iface the address of the interface where the message was sent from.
///
void
RoutingProtocol::ProcessMid (const lsr::MessageHeader &msg,
                             const Ipv4Address &senderIface)
{
  const lsr::MessageHeader::Mid &mid = msg.GetMid ();
  Time now = Simulator::Now ();
...
///
/// \param msg the %LSR message which contains the HNA message.
/// \param sender_iface the address of the interface where the message was sent from.
///
void
RoutingProtocol::ProcessHna (const lsr::MessageHeader &msg,
                             const Ipv4Address &senderIface)
{

  const lsr::MessageHeader::Hna &hna = msg.GetHna ();
  Time now = Simulator::Now ();
  ...
  for (std::vector<lsr::MessageHeader::Hna::Association>::const_iterator it = hna.associations.begin ();
...
///
/// \brief LSR's default forwarding algorithm.
///
/// See \RFC{3626} for details.
///
/// \param p the %LSR packet which has been received.
/// \param msg the %LSR message which must be forwarded.
...
///
void
RoutingProtocol::ForwardDefault (lsr::MessageHeader lsrMessage,
                                 ...
                                 const Ipv4Address &senderAddress)
{
  ...
  if (duplicated != NULL && duplicated->retransmitted)
    {
      NS_LOG_LOGIC (Simulator::Now () << "Node " << m_mainAddress << " does not forward a message received"
                    " from " << lsrMessage.GetOriginatorAddress () << " because it is duplicated");
      return;
    }
  ...
  if (duplicated != NULL)
    {
      duplicated->expirationTime = now + LSR_DUP_HOLD_TIME;
  ...
  // ...or create a new one
  else
    {
      ...
      newDup.address = lsrMessage.GetOriginatorAddress ();
      newDup.sequenceNumber = lsrMessage.GetMessageSequenceNumber ();
      newDup.expirationTime = now + LSR_DUP_HOLD_TIME;
      ...
      Simulator::Schedule (LSR_DUP_HOLD_TIME,
...
///
/// \brief Enques an %LSR message which will be sent with a delay of (0, delay].
///
/// This buffering system is used in order to piggyback several %LSR messages in
/// a same %LSR packet.
///
/// \param msg the %LSR message which must be sent.
/// \param delay maximum delay the %LSR message is going to be buffered.
///
void
RoutingProtocol::QueueMessage (const lsr::MessageHeader &message, Time delay)
{
...
{
  NS_LOG_DEBUG ("LSR node " << m_mainAddress << " sending a LSR packet");

  // Add a header
  lsr::PacketHeader header;
  ...
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator i =
         m_socketAddresses.begin (); i != m_socketAddresses.end (); i++)
    {
      ...
      i->first->SendTo (packet, 0, InetSocketAddress (bcast, LSR_PORT_NUMBER));
    }
}

///
/// \brief Creates as many %LSR packets as needed in order to send all buffered
/// %LSR messages.
///
/// Maximum number of messages which can be contained in an %LSR packet is
/// dictated by LSR_MAX_MSGS constant.
///
void
RoutingProtocol::SendQueuedMessages ()
{
  ...
  NS_LOG_DEBUG ("Lsr node " << m_mainAddress << ": SendQueuedMessages");

  MessageList msglist;

  for (std::vector<lsr::MessageHeader>::const_iterator message = m_queuedMessages.begin ();
...
///
/// \brief Creates a new %LSR HELLO message which is buffered for being sent later on.
///
void
RoutingProtocol::SendHello ()
{
  ...
  lsr::MessageHeader msg;
  Time now = Simulator::Now ();

  msg.SetVTime (LSR_NEIGHB_HOLD_TIME);
  msg.SetOriginatorAddress (m_mainAddress);
  msg.SetTimeToLive (1);
  msg.SetHopCount (0);
  msg.SetMessageSequenceNumber (GetMessageSequenceNumber ());
  lsr::MessageHeader::Hello &hello = msg.GetHello ();

  hello.SetHTime (m_helloInterval);
  hello.willingness = m_willingness;

  std::vector<lsr::MessageHeader::Hello::LinkMessage>
  ...
  for (LinkSet::const_iterator link_tuple = links.begin ();
       link_tuple != links.end (); link_tuple++)
    {
      ...
      lsr::MessageHeader::Hello::LinkMessage linkMessage;
  ...
  NS_LOG_DEBUG ("LSR HELLO message size: " << int (msg.GetSerializedSize ())
                                            << " (with " << int (linkMessages.size ()) << " link messages)");
  QueueMessage (msg, JITTER);
}

///
/// \brief Creates a new %LSR TC message which is buffered for being sent later on.
///
void
RoutingProtocol::SendTc ()
{
  ...
  msg.SetVTime (LSR_TOP_HOLD_TIME);
  msg.SetOriginatorAddress (m_mainAddress);
  msg.SetTimeToLive (255);
  msg.SetHopCount (0);
  msg.SetMessageSequenceNumber (GetMessageSequenceNumber ());

  lsr::MessageHeader::Tc &tc = msg.GetTc ();
  tc.ansn = m_ansn;
...
///
/// \brief Creates a new %LSR MID message which is buffered for being sent later on.
///
void
RoutingProtocol::SendMid ()
{
  lsr::MessageHeader msg;
  lsr::MessageHeader::Mid &mid = msg.GetMid ();

  // A node which has only a single interface address participating in
  // the MANET (i.e., running LSR), MUST NOT generate any MID
  // message.

  // A node with several interfaces, where only one is participating
  // in the MANET and running LSR (e.g., a node is connected to a
  // wired network as well as to a MANET) MUST NOT generate any MID
  // messages.

  // A node with several interfaces, where more than one is
  // participating in the MANET and running LSR MUST generate MID
  ...
  msg.SetVTime (LSR_MID_HOLD_TIME);
  msg.SetOriginatorAddress (m_mainAddress);
  msg.SetTimeToLive (255);
  msg.SetHopCount (0);
  msg.SetMessageSequenceNumber (GetMessageSequenceNumber ());
...
///
/// \brief Creates a new %LSR HNA message which is buffered for being sent later on.
///
void
RoutingProtocol::SendHna ()
{
  ...
  msg.SetVTime (LSR_HNA_HOLD_TIME);
  msg.SetOriginatorAddress (m_mainAddress);
  msg.SetTimeToLive (255);
  msg.SetHopCount (0);
  msg.SetMessageSequenceNumber (GetMessageSequenceNumber ());
  lsr::MessageHeader::Hna &hna = msg.GetHna ();

  std::vector<lsr::MessageHeader::Hna::Association> &associations = hna.associations;
  ...
  for (Associations::const_iterator it = localHnaAssociations.begin ();
    ...
    {
      lsr::MessageHeader::Hna::Association assoc = { it->networkAddr, it->netmask};
...
///        local HNA associations to be sent by the node via HNA messages.
///        If this tuple does not exist, nothing is done (see "LsrState::EraseAssociation()").
///
void
...
///        to the LSR routing protocol. Entries from this associated
///        routing table that use non-lsr outgoing interfaces are added
...
RoutingProtocol::SetRoutingTableAssociation (Ptr<Ipv4StaticRouting> routingTable)
{
      ...
      for (uint32_t i = 0; i < m_routingTableAssociation->GetNRoutes (); i++)
        {
          ...
          // If the outgoing interface for this route is a non-lsr interface
          if (UsesNonLsrOutgoingInterface (route))
            {
  ...
  // add the routes using non-lsr outgoing interfaces to the list
...
///
/// \brief Tests whether or not the specified route uses a non-LSR outgoing interface.
///        Returns true if the outgoing interface of the specified route is a non-LSR interface.
///        Returns false otherwise.
///
bool
RoutingProtocol::UsesNonLsrOutgoingInterface (const Ipv4RoutingTableEntry &route)
{
  std::set<uint32_t>::const_iterator ci = m_interfaceExclusions.find (route.GetInterface ());
  // The outgoing interface is a non-LSR interface if a match is found
...
void
RoutingProtocol::LinkSensing (const lsr::MessageHeader &msg,
                              const lsr::MessageHeader::Hello &hello,
                              const Ipv4Address &receiverIface,
                              const Ipv4Address &senderIface)
{
  Time now = Simulator::Now ();
  bool updated = false;
  bool created = false;
  NS_LOG_DEBUG ("@" << now.GetSeconds () << ": Lsr node " << m_mainAddress
  ...
  for (std::vector<lsr::MessageHeader::Hello::LinkMessage>::const_iterator linkMessage =
       ...
       linkMessage++)
    {
      ...
      switch (lt)
        {
        case LSR_UNSPEC_LINK: linkTypeName = "UNSPEC_LINK"; break;
        case LSR_ASYM_LINK: linkTypeName = "ASYM_LINK"; break;
        case LSR_SYM_LINK: linkTypeName = "SYM_LINK"; break;
        case LSR_LOST_LINK: linkTypeName = "LOST_LINK"; break;
      ...
      switch (nt)
        {
        case LSR_NOT_NEIGH: neighborTypeName = "NOT_NEIGH"; break;
        case LSR_SYM_NEIGH: neighborTypeName = "SYM_NEIGH"; break;
        case LSR_MPR_NEIGH: neighborTypeName = "MPR_NEIGH"; break;
      ...
      if ((lt == LSR_SYM_LINK && nt == LSR_NOT_NEIGH) ||
          (nt != LSR_SYM_NEIGH && nt != LSR_MPR_NEIGH
           && nt != LSR_NOT_NEIGH))
        {
          ...
          if (*neighIfaceAddr == receiverIface)
            {
              ...
              else if (lt == LSR_SYM_LINK || lt == LSR_ASYM_LINK)
                {
                  ...
                  link_tuple->time = link_tuple->symTime + LSR_NEIGHB_HOLD_TIME;
                  updated = true;
                }
              else
                {
...
void
RoutingProtocol::PopulateNeighborSet (const lsr::MessageHeader &msg,
                                      const lsr::MessageHeader::Hello &hello)
{
...
void
RoutingProtocol::PopulateTwoHopNeighborSet (const lsr::MessageHeader &msg,
                                            const lsr::MessageHeader::Hello &hello)
{
  Time now = Simulator::Now ();

  NS_LOG_DEBUG ("Lsr node " << m_mainAddress << ": PopulateTwoHopNeighborSet BEGIN");
  ...
  for (LinkSet::const_iterator link_tuple = m_state.GetLinks ().begin ();
       link_tuple != m_state.GetLinks ().end (); link_tuple++)
    {
      ...
      typedef std::vector<lsr::MessageHeader::Hello::LinkMessage> LinkMessageVec;
           ...
           linkMessage != hello.linkMessages.end (); linkMessage++)
        {
          ...
          for (std::vector<Ipv4Address>::const_iterator nb2hop_addr_iter =
               ...
               nb2hop_addr_iter++)
            {
              ...
              if (neighborType == LSR_SYM_NEIGH || neighborType == LSR_MPR_NEIGH)
                {
              ...
              else if (neighborType == LSR_NOT_NEIGH)
                {
  ...
  NS_LOG_DEBUG ("Lsr node " << m_mainAddress << ": PopulateTwoHopNeighborSet END");
}
...
/// a new received HELLO message (following \RFC{3626}).
void
RoutingProtocol::PopulateMprSelectorSet (const lsr::MessageHeader &msg,
                                         const lsr::MessageHeader::Hello &hello)
{
  NS_LOG_FUNCTION (this);
...
LSR::mac_failed (Ptr<Packet> p) {
  ...
  LSR_link_tuple* link_tuple = state_.find_link_tuple (ch->next_hop ());
  if (link_tuple != NULL) {
      link_tuple->lost_time () = now + LSR_NEIGHB_HOLD_TIME;
      link_tuple->time ()      = now + LSR_NEIGHB_HOLD_TIME;
      nb_loss (link_tuple);
    }
  drop (p, DROP_RTR_MAC_CALLBACK);
}
...
RoutingProtocol::NeighborLoss (const LinkTuple &tuple)
{
  ...
  LinkTupleUpdated (tuple, LSR_WILL_DEFAULT);
...
//         Simulator::Now (),
//         LSR::node_id(ra_addr()),
//         LSR::node_id(tuple->neighborMainAddr),
//         ((tuple->status() == LSR_STATUS_SYM) ? "sym" : "not_sym"));
...
//         LSR::node_id(ra_addr()),
//         LSR::node_id(tuple->neighborMainAddr),
//         LSR::node_id(tuple->twoHopNeighborAddr));
...
{
  m_ansn = (m_ansn + 1) % (LSR_MAX_SEQ_NUM + 1);
}
...
//         Simulator::Now (),
//         LSR::node_id(ra_addr()),
//         LSR::node_id(tuple->dest_addr()),
//         LSR::node_id(tuple->last_addr()),
//         tuple->seq());
...
{
  m_packetSequenceNumber = (m_packetSequenceNumber + 1) % (LSR_MAX_SEQ_NUM + 1);
...
{
  m_messageSequenceNumber = (m_messageSequenceNumber + 1) % (LSR_MAX_SEQ_NUM + 1);
...
///
/// The task of actually removing the tuple is left to the LSR agent.
///
  ...
  if (Lookup (header.GetDestination (), entry1) != 0)
    {
      ...
      if (oif && m_ipv4->GetInterfaceForDevice (oif) != static_cast<int> (interfaceIdx))
        {
          ...
          NS_LOG_DEBUG ("Lsr node " << m_mainAddress 
      ...
      if (numOifAddresses == 1) {
          ...
          /// \todo Implment IP aliasing and LSR
          NS_FATAL_ERROR ("XXX Not implemented yet:  IP aliasing and LSR");
        }
