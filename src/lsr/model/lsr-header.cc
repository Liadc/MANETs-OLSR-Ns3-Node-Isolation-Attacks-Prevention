...
#define LSR_MSG_HEADER_SIZE 12
#define LSR_PKT_HEADER_SIZE 4

namespace ns3 {
namespace lsr {


NS_LOG_COMPONENT_DEFINE ("LsrHeader");
...
SecondsToEmf (double seconds)
{
  ...
  for (b = 0; (seconds/LSR_C) >= (1 << b); ++b)
    ;
  NS_ASSERT ((seconds/LSR_C) < (1 << b));
  b--;
  NS_ASSERT ((seconds/LSR_C) >= (1 << b));

  // compute the expression 16*(T/(C*(2^b))-1), which may not be a integer
  double tmp = 16*(seconds/(LSR_C*(1<<b))-1);
...
///
/// \param lsr_format number of seconds in mantissa/exponent format.
/// \return the decimal number of seconds.
///
double
EmfToSeconds (uint8_t lsrFormat)
{
  int a = (lsrFormat >> 4);
  int b = (lsrFormat & 0xf);
  // value = C*(1+a/16)*2^b [in seconds]
  return LSR_C * (1 + a/16.0) * (1 << b);
}
...
{
  static TypeId tid = TypeId ("ns3::lsr::PacketHeader")
    .SetParent<Header> ()
...
{
  static TypeId tid = TypeId ("ns3::lsr::MessageHeader")
    .SetParent<Header> ()
...
{
  uint32_t size = LSR_MSG_HEADER_SIZE;
...
MessageHeader::Deserialize (Buffer::Iterator start)
{
  ...
  size = LSR_MSG_HEADER_SIZE;
  switch (m_messageType)
    {
    case MID_MESSAGE:
      size += m_message.mid.Deserialize (i, m_messageSize - LSR_MSG_HEADER_SIZE);
      break;
    case HELLO_MESSAGE:
      size += m_message.hello.Deserialize (i, m_messageSize - LSR_MSG_HEADER_SIZE);
      break;
    case TC_MESSAGE:
      size += m_message.tc.Deserialize (i, m_messageSize - LSR_MSG_HEADER_SIZE);
      break;
    case HNA_MESSAGE:
      size += m_message.hna.Deserialize (i, m_messageSize - LSR_MSG_HEADER_SIZE);
      break;
    default:
      NS_ASSERT (false);
    }
