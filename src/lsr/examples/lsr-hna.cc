...
//
// One way is to use lsr::RoutingProtocol::SetRoutingTableAssociation ()
// to use which you may run:
//
// ./waf --run "lsr-hna --assocMethod1=1"
//
// The other way is to use lsr::RoutingProtocol::AddHostNetworkAssociation ()
// to use which you may run:
//
// ./waf --run "lsr-hna --assocMethod2=1"
//
...
NS_LOG_COMPONENT_DEFINE ("LsrHna");
...
int main (int argc, char *argv[])
{
  ...
  NetDeviceContainer devices = wifi.Install (wifiPhy, wifiMac, lsrNodes);

  CsmaHelper csma;
  csma.SetChannelAttribute ("DataRate", DataRateValue (DataRate (5000000)));
  csma.SetChannelAttribute ("Delay", TimeValue (MilliSeconds (2)));
  NetDeviceContainer csmaDevices = csma.Install (NodeContainer (csmaNodes.Get (0), lsrNodes.Get (1)));
  ...
  mobility.Install (lsrNodes);

  LsrHelper lsr;

  // Specify Node B's csma device as a non-LSR device.
  lsr.ExcludeInterface (lsrNodes.Get (1), 2);
  ...
  InternetStackHelper internet_lsr;
  internet_lsr.SetRoutingHelper (list); // has effect on the next Install ()
  internet_lsr.Install (lsrNodes);
  ...
  Ptr<Socket> source = Socket::CreateSocket (lsrNodes.Get (0), tid);
  InetSocketAddress remote = InetSocketAddress (Ipv4Address ("172.16.1.1"), 80);
  source->Connect (remote);

  // Obtain lsr::RoutingProtocol instance of gateway node
  // (namely, node B) and add the required association
  Ptr<Ipv4> stack = lsrNodes.Get (1)->GetObject<Ipv4> ();
  Ptr<Ipv4RoutingProtocol> rp_Gw = (stack->GetRoutingProtocol ());
  Ptr<Ipv4ListRouting> lrp_Gw = DynamicCast<Ipv4ListRouting> (rp_Gw);

  Ptr<lsr::RoutingProtocol> lsrrp_Gw;

  for (uint32_t i = 0; i < lrp_Gw->GetNRoutingProtocols ();  i++)
    {
      int16_t priority;
      Ptr<Ipv4RoutingProtocol> temp = lrp_Gw->GetRoutingProtocol (i, priority);
      if (DynamicCast<lsr::RoutingProtocol> (temp))
        {
          lsrrp_Gw = DynamicCast<lsr::RoutingProtocol> (temp);
        }
    }
  ...
  if (assocMethod1)
    {
      ...
      // which are associated with non-LSR interfaces specified above.
      hnaEntries->AddNetworkRouteTo (Ipv4Address ("172.16.1.0"), Ipv4Mask ("255.255.255.0"), uint32_t (2), uint32_t (1));
      lsrrp_Gw->SetRoutingTableAssociation (hnaEntries);
    }
 
  if (assocMethod2)
    {
      // Specify the required associations directly.
      lsrrp_Gw->AddHostNetworkAssociation (Ipv4Address ("172.16.1.0"), Ipv4Mask ("255.255.255.0"));
    }

  // Tracing
  wifiPhy.EnablePcap ("lsr-hna", devices);
  csma.EnablePcap ("lsr-hna", csmaDevices, false);
