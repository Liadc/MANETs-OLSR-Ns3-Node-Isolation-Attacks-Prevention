...
//
// One way is to use solsr::RoutingProtocol::SetRoutingTableAssociation ()
// to use which you may run:
//
// ./waf --run "solsr-hna --assocMethod1=1"
//
// The other way is to use solsr::RoutingProtocol::AddHostNetworkAssociation ()
// to use which you may run:
//
// ./waf --run "solsr-hna --assocMethod2=1"
//
...
NS_LOG_COMPONENT_DEFINE ("SOlsrHna");
...
int main (int argc, char *argv[])
{
  ...
  NetDeviceContainer devices = wifi.Install (wifiPhy, wifiMac, solsrNodes);

  CsmaHelper csma;
  csma.SetChannelAttribute ("DataRate", DataRateValue (DataRate (5000000)));
  csma.SetChannelAttribute ("Delay", TimeValue (MilliSeconds (2)));
  NetDeviceContainer csmaDevices = csma.Install (NodeContainer (csmaNodes.Get (0), solsrNodes.Get (1)));
  ...
  mobility.Install (solsrNodes);

  SOlsrHelper solsr;

  // Specify Node B's csma device as a non-SOLSR device.
  solsr.ExcludeInterface (solsrNodes.Get (1), 2);
  ...
  InternetStackHelper internet_solsr;
  internet_solsr.SetRoutingHelper (list); // has effect on the next Install ()
  internet_solsr.Install (solsrNodes);
  ...
  Ptr<Socket> source = Socket::CreateSocket (solsrNodes.Get (0), tid);
  InetSocketAddress remote = InetSocketAddress (Ipv4Address ("172.16.1.1"), 80);
  source->Connect (remote);

  // Obtain solsr::RoutingProtocol instance of gateway node
  // (namely, node B) and add the required association
  Ptr<Ipv4> stack = solsrNodes.Get (1)->GetObject<Ipv4> ();
  Ptr<Ipv4RoutingProtocol> rp_Gw = (stack->GetRoutingProtocol ());
  Ptr<Ipv4ListRouting> lrp_Gw = DynamicCast<Ipv4ListRouting> (rp_Gw);

  Ptr<solsr::RoutingProtocol> solsrrp_Gw;

  for (uint32_t i = 0; i < lrp_Gw->GetNRoutingProtocols ();  i++)
    {
      int16_t priority;
      Ptr<Ipv4RoutingProtocol> temp = lrp_Gw->GetRoutingProtocol (i, priority);
      if (DynamicCast<solsr::RoutingProtocol> (temp))
        {
          solsrrp_Gw = DynamicCast<solsr::RoutingProtocol> (temp);
        }
    }
  ...
  if (assocMethod1)
    {
      ...
      // which are associated with non-SOLSR interfaces specified above.
      hnaEntries->AddNetworkRouteTo (Ipv4Address ("172.16.1.0"), Ipv4Mask ("255.255.255.0"), uint32_t (2), uint32_t (1));
      solsrrp_Gw->SetRoutingTableAssociation (hnaEntries);
    }
 
  if (assocMethod2)
    {
      // Specify the required associations directly.
      solsrrp_Gw->AddHostNetworkAssociation (Ipv4Address ("172.16.1.0"), Ipv4Mask ("255.255.255.0"));
    }

  // Tracing
  wifiPhy.EnablePcap ("solsr-hna", devices);
  csma.EnablePcap ("solsr-hna", csmaDevices, false);
