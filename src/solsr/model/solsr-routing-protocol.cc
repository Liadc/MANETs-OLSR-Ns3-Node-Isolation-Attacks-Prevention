...
///
/// \brief Implementation of SOLSR agent and related classes.
///
/// This is the main file of this software because %SOLSR's behaviour is
/// implemented here.
///
...
///
/// We only use this value in order to define SOLSR_NEIGHB_HOLD_TIME.
///
#define SOLSR_REFRESH_INTERVAL   m_helloInterval


/********** Holding times **********/

/// Neighbor holding time.
#define SOLSR_NEIGHB_HOLD_TIME   Time (3 * SOLSR_REFRESH_INTERVAL)
/// Top holding time.
#define SOLSR_TOP_HOLD_TIME      Time (3 * m_tcInterval)
/// Dup holding time.
#define SOLSR_DUP_HOLD_TIME      Seconds (30)
/// MID holding time.
#define SOLSR_MID_HOLD_TIME      Time (3 * m_midInterval)
/// HNA holding time.
#define SOLSR_HNA_HOLD_TIME      Time (3 * m_hnaInterval)

/********** Link types **********/

/// Unspecified link type.
#define SOLSR_UNSPEC_LINK        0
/// Asymmetric link type.
#define SOLSR_ASYM_LINK          1
/// Symmetric link type.
#define SOLSR_SYM_LINK           2
/// Lost link type.
#define SOLSR_LOST_LINK          3

/********** Neighbor types **********/

/// Not neighbor type.
#define SOLSR_NOT_NEIGH          0
/// Symmetric neighbor type.
#define SOLSR_SYM_NEIGH          1
/// Asymmetric neighbor type.
#define SOLSR_MPR_NEIGH          2
...
#define SOLSR_MAXJITTER          (m_helloInterval.GetSeconds () / 4)
/// Maximum allowed sequence number.
#define SOLSR_MAX_SEQ_NUM        65535
/// Random number between [0-SOLSR_MAXJITTER] used to jitter SOLSR packet transmission.
#define JITTER (Seconds (m_uniformRandomVariable->GetValue (0, SOLSR_MAXJITTER)))


#define SOLSR_PORT_NUMBER 698
/// Maximum number of messages per packet.
#define SOLSR_MAX_MSGS           64

/// Maximum number of hellos per message (4 possible link types * 3 possible nb types).
#define SOLSR_MAX_HELLOS         12

/// Maximum number of addresses advertised on a message.
#define SOLSR_MAX_ADDRS          64


namespace ns3 {
namespace solsr {

NS_LOG_COMPONENT_DEFINE ("SOlsrRoutingProtocol");
...
{
  static TypeId tid = TypeId ("ns3::solsr::RoutingProtocol")
    ...
    .AddAttribute ("Willingness", "Willingness of a node to carry and forward traffic for other nodes.",
                   EnumValue (SOLSR_WILL_DEFAULT),
                   MakeEnumAccessor (&RoutingProtocol::m_willingness),
                   MakeEnumChecker (SOLSR_WILL_NEVER, "never",
                                    SOLSR_WILL_LOW, "low",
                                    SOLSR_WILL_DEFAULT, "default",
                                    SOLSR_WILL_HIGH, "high",
                                    SOLSR_WILL_ALWAYS, "always"))
    .AddTraceSource ("Rx", "Receive SOLSR packet.",
                     MakeTraceSourceAccessor (&RoutingProtocol::m_rxPacketTrace))
    .AddTraceSource ("Tx", "Send SOLSR packet.",
                     MakeTraceSourceAccessor (&RoutingProtocol::m_txPacketTrace))
    .AddTraceSource ("RoutingTableChanged", "The SOLSR routing table has changed.",
...
RoutingProtocol::SetIpv4 (Ptr<Ipv4> ipv4)
{
  NS_ASSERT (ipv4 != 0);
  NS_ASSERT (m_ipv4 == 0);
  NS_LOG_DEBUG ("Created solsr::RoutingProtocol");
  ...
  m_packetSequenceNumber = SOLSR_MAX_SEQ_NUM;
  m_messageSequenceNumber = SOLSR_MAX_SEQ_NUM;
  m_ansn = SOLSR_MAX_SEQ_NUM;
...
void RoutingProtocol::DoInitialize ()
{
  ...
  NS_LOG_DEBUG ("Starting SOLSR on node " << m_mainAddress);

  Ipv4Address loopback ("127.0.0.1");

  bool canRunSOlsr = false;
  for (uint32_t i = 0; i < m_ipv4->GetNInterfaces (); i++)
    {
      Ipv4Address addr = m_ipv4->GetAddress (i, 0).GetLocal ();
      if (addr == loopback)
      ...
      InetSocketAddress inetAddr (m_ipv4->GetAddress (i, 0).GetLocal (), SOLSR_PORT_NUMBER);
      socket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvSOlsr,  this));
      if (socket->Bind (inetAddr))
        {
          NS_FATAL_ERROR ("Failed to bind() SOLSR socket");
        }
  ...
  if(canRunSOlsr)
    {
      ...
      NS_LOG_DEBUG ("SOLSR on node " << m_mainAddress << " started");
    }
}
...
//
// \brief Processes an incoming %SOLSR packet following \RFC{3626} specification.
void
RoutingProtocol::RecvSOlsr (Ptr<Socket> socket)
{
  ...
  NS_LOG_DEBUG ("SOLSR node " << m_mainAddress << " received a SOLSR packet from "
                             << senderIfaceAddr << " to " << receiverIfaceAddr);

  // All routing messages are sent from and to port RT_PORT,
  // so we check it.
  NS_ASSERT (inetSourceAddr.GetPort () == SOLSR_PORT_NUMBER);

  Ptr<Packet> packet = receivedPacket;

  solsr::PacketHeader solsrPacketHeader;
  packet->RemoveHeader (solsrPacketHeader);
  NS_ASSERT (solsrPacketHeader.GetPacketLength () >= solsrPacketHeader.GetSerializedSize ());
  uint32_t sizeLeft = solsrPacketHeader.GetPacketLength () - solsrPacketHeader.GetSerializedSize ();
  ...
  while (sizeLeft)
    {
      ...
      NS_LOG_DEBUG ("SOlsr Msg received with type "
  ...
  m_rxPacketTrace (solsrPacketHeader, messages);
...
//         }
      ...
      if (duplicated == NULL)
        {
          switch (messageHeader.GetMessageType ())
            {
            case solsr::MessageHeader::HELLO_MESSAGE:
              NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                            << "s SOLSR node " << m_mainAddress
            ...
            case solsr::MessageHeader::TC_MESSAGE:
              NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                            << "s SOLSR node " << m_mainAddress
            ...
            default:
              NS_LOG_DEBUG ("SOLSR message type " <<
        ...
        {
          NS_LOG_DEBUG ("SOLSR message is duplicated, not reading it.");
      ...
      if (do_forwarding)
        {
          ...
          if (messageHeader.GetMessageType ()  != solsr::MessageHeader::HELLO_MESSAGE)
            {
  ...
  // After processing all SOLSR messages, we must recompute the routing table
...
RoutingProtocol::MprComputation ()
{
      ...
      for (NeighborSet::const_iterator neigh = N.begin ();
           neigh != N.end (); neigh++)
        {
          if (neigh->neighborMainAddr == twoHopNeigh->neighborMainAddr)
            {
              if (neigh->willingness == SOLSR_WILL_NEVER)
                {
                  ok = false;
                  break;
                }
  ...
  for (NeighborSet::const_iterator neighbor = N.begin (); neighbor != N.end (); neighbor++)
    {
      if (neighbor->willingness == SOLSR_WILL_ALWAYS)
        {
      ...
      for (NeighborSet::const_iterator neighbor = neighborSet.begin ();
           neighbor != neighborSet.end (); neighbor++)
        {
          if (neighbor->neighborMainAddr == nb2hop_tuple.neighborMainAddr
              && neighbor->willingness != SOLSR_WILL_NEVER)
            {
              nb2hopOk = true;
              break;
            }
...
///
/// \param msg the %SOLSR message which contains the HELLO message.
/// \param receiver_iface the address of the interface where the message was received from.
/// \param sender_iface the address of the interface where the message was sent from.
///
void
RoutingProtocol::ProcessHello (const solsr::MessageHeader &msg,
                               const Ipv4Address &receiverIface,
                               const Ipv4Address &senderIface)
{
  NS_LOG_FUNCTION (msg << receiverIface << senderIface);

  const solsr::MessageHeader::Hello &hello = msg.GetHello ();
...
#ifdef NS3_LOG_ENABLE
  {
    ...
    NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                  << "s ** BEGIN dump Link Set for SOLSR Node " << m_mainAddress);
    for (LinkSet::const_iterator link = links.begin (); link != links.end (); link++)
      {
        NS_LOG_DEBUG (*link);
      }
    NS_LOG_DEBUG ("** END dump Link Set for SOLSR Node " << m_mainAddress);

    const NeighborSet &neighbors = m_state.GetNeighbors ();
    NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                  << "s ** BEGIN dump Neighbor Set for SOLSR Node " << m_mainAddress);
    for (NeighborSet::const_iterator neighbor = neighbors.begin (); neighbor != neighbors.end (); neighbor++)
      {
        NS_LOG_DEBUG (*neighbor);
      }
    NS_LOG_DEBUG ("** END dump Neighbor Set for SOLSR Node " << m_mainAddress);
  }
#endif // NS3_LOG_ENABLE
...
#ifdef NS3_LOG_ENABLE
  {
    const TwoHopNeighborSet &twoHopNeighbors = m_state.GetTwoHopNeighbors ();
    NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                  << "s ** BEGIN dump TwoHopNeighbor Set for SOLSR Node " << m_mainAddress);
    for (TwoHopNeighborSet::const_iterator tuple = twoHopNeighbors.begin ();
         tuple != twoHopNeighbors.end (); tuple++)
      {
        NS_LOG_DEBUG (*tuple);
      }
    NS_LOG_DEBUG ("** END dump TwoHopNeighbor Set for SOLSR Node " << m_mainAddress);
  }
#endif // NS3_LOG_ENABLE
...
///
/// \param msg the %SOLSR message which contains the TC message.
/// \param sender_iface the address of the interface where the message was sent from.
///
void
RoutingProtocol::ProcessTc (const solsr::MessageHeader &msg,
                            const Ipv4Address &senderIface)
{
  const solsr::MessageHeader::Tc &tc = msg.GetTc ();
...
#ifdef NS3_LOG_ENABLE
  {
    const TopologySet &topology = m_state.GetTopologySet ();
    NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                  << "s ** BEGIN dump TopologySet for SOLSR Node " << m_mainAddress);
    for (TopologySet::const_iterator tuple = topology.begin ();
         tuple != topology.end (); tuple++)
      {
        NS_LOG_DEBUG (*tuple);
      }
    NS_LOG_DEBUG ("** END dump TopologySet Set for SOLSR Node " << m_mainAddress);
  }
#endif // NS3_LOG_ENABLE
}
...
///
/// \param msg the %SOLSR message which contains the MID message.
/// \param sender_iface the address of the interface where the message was sent from.
///
void
RoutingProtocol::ProcessMid (const solsr::MessageHeader &msg,
                             const Ipv4Address &senderIface)
{
  const solsr::MessageHeader::Mid &mid = msg.GetMid ();
  Time now = Simulator::Now ();
...
///
/// \param msg the %SOLSR message which contains the HNA message.
/// \param sender_iface the address of the interface where the message was sent from.
///
void
RoutingProtocol::ProcessHna (const solsr::MessageHeader &msg,
                             const Ipv4Address &senderIface)
{

  const solsr::MessageHeader::Hna &hna = msg.GetHna ();
  Time now = Simulator::Now ();
  ...
  for (std::vector<solsr::MessageHeader::Hna::Association>::const_iterator it = hna.associations.begin ();
...
///
/// \brief SOLSR's default forwarding algorithm.
///
/// See \RFC{3626} for details.
///
/// \param p the %SOLSR packet which has been received.
/// \param msg the %SOLSR message which must be forwarded.
...
///
void
RoutingProtocol::ForwardDefault (solsr::MessageHeader solsrMessage,
                                 ...
                                 const Ipv4Address &senderAddress)
{
  ...
  if (duplicated != NULL && duplicated->retransmitted)
    {
      NS_LOG_LOGIC (Simulator::Now () << "Node " << m_mainAddress << " does not forward a message received"
                    " from " << solsrMessage.GetOriginatorAddress () << " because it is duplicated");
      return;
    }
  ...
  if (solsrMessage.GetTimeToLive () > 1)
    {
      const MprSelectorTuple *mprselTuple =
        m_state.FindMprSelectorTuple (GetMainAddress (senderAddress));
      if (mprselTuple != NULL)
        {
          solsrMessage.SetTimeToLive (solsrMessage.GetTimeToLive () - 1);
          solsrMessage.SetHopCount (solsrMessage.GetHopCount () + 1);
          // We have to introduce a random delay to avoid
          // synchronization with neighbors.
          QueueMessage (solsrMessage, JITTER);
          retransmitted = true;
        }
    }
  ...
  if (duplicated != NULL)
    {
      duplicated->expirationTime = now + SOLSR_DUP_HOLD_TIME;
  ...
  // ...or create a new one
  else
    {
      ...
      newDup.address = solsrMessage.GetOriginatorAddress ();
      newDup.sequenceNumber = solsrMessage.GetMessageSequenceNumber ();
      newDup.expirationTime = now + SOLSR_DUP_HOLD_TIME;
      ...
      Simulator::Schedule (SOLSR_DUP_HOLD_TIME,
...
///
/// \brief Enques an %SOLSR message which will be sent with a delay of (0, delay].
///
/// This buffering system is used in order to piggyback several %SOLSR messages in
/// a same %SOLSR packet.
///
/// \param msg the %SOLSR message which must be sent.
/// \param delay maximum delay the %SOLSR message is going to be buffered.
///
void
RoutingProtocol::QueueMessage (const solsr::MessageHeader &message, Time delay)
{
...
{
  NS_LOG_DEBUG ("SOLSR node " << m_mainAddress << " sending a SOLSR packet");

  // Add a header
  solsr::PacketHeader header;
  ...
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator i =
         m_socketAddresses.begin (); i != m_socketAddresses.end (); i++)
    {
      ...
      i->first->SendTo (packet, 0, InetSocketAddress (bcast, SOLSR_PORT_NUMBER));
    }
}

///
/// \brief Creates as many %SOLSR packets as needed in order to send all buffered
/// %SOLSR messages.
///
/// Maximum number of messages which can be contained in an %SOLSR packet is
/// dictated by SOLSR_MAX_MSGS constant.
///
void
RoutingProtocol::SendQueuedMessages ()
{
  ...
  NS_LOG_DEBUG ("SOlsr node " << m_mainAddress << ": SendQueuedMessages");

  MessageList msglist;

  for (std::vector<solsr::MessageHeader>::const_iterator message = m_queuedMessages.begin ();
...
///
/// \brief Creates a new %SOLSR HELLO message which is buffered for being sent later on.
///
void
RoutingProtocol::SendHello ()
{
  ...
  solsr::MessageHeader msg;
  Time now = Simulator::Now ();

  msg.SetVTime (SOLSR_NEIGHB_HOLD_TIME);
  msg.SetOriginatorAddress (m_mainAddress);
  msg.SetTimeToLive (1);
  msg.SetHopCount (0);
  msg.SetMessageSequenceNumber (GetMessageSequenceNumber ());
  solsr::MessageHeader::Hello &hello = msg.GetHello ();

  hello.SetHTime (m_helloInterval);
  hello.willingness = m_willingness;

  std::vector<solsr::MessageHeader::Hello::LinkMessage>
  ...
  for (LinkSet::const_iterator link_tuple = links.begin ();
       link_tuple != links.end (); link_tuple++)
    {
      ...
      solsr::MessageHeader::Hello::LinkMessage linkMessage;
  ...
  NS_LOG_DEBUG ("SOLSR HELLO message size: " << int (msg.GetSerializedSize ())
                                            << " (with " << int (linkMessages.size ()) << " link messages)");
  QueueMessage (msg, JITTER);
}

///
/// \brief Creates a new %SOLSR TC message which is buffered for being sent later on.
///
void
RoutingProtocol::SendTc ()
{
  ...
  msg.SetVTime (SOLSR_TOP_HOLD_TIME);
  msg.SetOriginatorAddress (m_mainAddress);
  msg.SetTimeToLive (255);
  msg.SetHopCount (0);
  msg.SetMessageSequenceNumber (GetMessageSequenceNumber ());

  solsr::MessageHeader::Tc &tc = msg.GetTc ();
  tc.ansn = m_ansn;
...
///
/// \brief Creates a new %SOLSR MID message which is buffered for being sent later on.
///
void
RoutingProtocol::SendMid ()
{
  solsr::MessageHeader msg;
  solsr::MessageHeader::Mid &mid = msg.GetMid ();

  // A node which has only a single interface address participating in
  // the MANET (i.e., running SOLSR), MUST NOT generate any MID
  // message.

  // A node with several interfaces, where only one is participating
  // in the MANET and running SOLSR (e.g., a node is connected to a
  // wired network as well as to a MANET) MUST NOT generate any MID
  // messages.

  // A node with several interfaces, where more than one is
  // participating in the MANET and running SOLSR MUST generate MID
  ...
  msg.SetVTime (SOLSR_MID_HOLD_TIME);
  msg.SetOriginatorAddress (m_mainAddress);
  msg.SetTimeToLive (255);
  msg.SetHopCount (0);
  msg.SetMessageSequenceNumber (GetMessageSequenceNumber ());
...
///
/// \brief Creates a new %SOLSR HNA message which is buffered for being sent later on.
///
void
RoutingProtocol::SendHna ()
{
  ...
  msg.SetVTime (SOLSR_HNA_HOLD_TIME);
  msg.SetOriginatorAddress (m_mainAddress);
  msg.SetTimeToLive (255);
  msg.SetHopCount (0);
  msg.SetMessageSequenceNumber (GetMessageSequenceNumber ());
  solsr::MessageHeader::Hna &hna = msg.GetHna ();

  std::vector<solsr::MessageHeader::Hna::Association> &associations = hna.associations;
  ...
  for (Associations::const_iterator it = localHnaAssociations.begin ();
    ...
    {
      solsr::MessageHeader::Hna::Association assoc = { it->networkAddr, it->netmask};
...
///        local HNA associations to be sent by the node via HNA messages.
///        If this tuple does not exist, nothing is done (see "SOlsrState::EraseAssociation()").
///
void
...
///        to the SOLSR routing protocol. Entries from this associated
///        routing table that use non-solsr outgoing interfaces are added
...
RoutingProtocol::SetRoutingTableAssociation (Ptr<Ipv4StaticRouting> routingTable)
{
      ...
      for (uint32_t i = 0; i < m_routingTableAssociation->GetNRoutes (); i++)
        {
          ...
          // If the outgoing interface for this route is a non-solsr interface
          if (UsesNonSOlsrOutgoingInterface (route))
            {
  ...
  // add the routes using non-solsr outgoing interfaces to the list
...
///
/// \brief Tests whether or not the specified route uses a non-SOLSR outgoing interface.
///        Returns true if the outgoing interface of the specified route is a non-SOLSR interface.
///        Returns false otherwise.
///
bool
RoutingProtocol::UsesNonSOlsrOutgoingInterface (const Ipv4RoutingTableEntry &route)
{
  std::set<uint32_t>::const_iterator ci = m_interfaceExclusions.find (route.GetInterface ());
  // The outgoing interface is a non-SOLSR interface if a match is found
...
void
RoutingProtocol::LinkSensing (const solsr::MessageHeader &msg,
                              const solsr::MessageHeader::Hello &hello,
                              const Ipv4Address &receiverIface,
                              const Ipv4Address &senderIface)
{
  Time now = Simulator::Now ();
  bool updated = false;
  bool created = false;
  NS_LOG_DEBUG ("@" << now.GetSeconds () << ": SOlsr node " << m_mainAddress
  ...
  for (std::vector<solsr::MessageHeader::Hello::LinkMessage>::const_iterator linkMessage =
       ...
       linkMessage++)
    {
      ...
      switch (lt)
        {
        case SOLSR_UNSPEC_LINK: linkTypeName = "UNSPEC_LINK"; break;
        case SOLSR_ASYM_LINK: linkTypeName = "ASYM_LINK"; break;
        case SOLSR_SYM_LINK: linkTypeName = "SYM_LINK"; break;
        case SOLSR_LOST_LINK: linkTypeName = "LOST_LINK"; break;
      ...
      switch (nt)
        {
        case SOLSR_NOT_NEIGH: neighborTypeName = "NOT_NEIGH"; break;
        case SOLSR_SYM_NEIGH: neighborTypeName = "SYM_NEIGH"; break;
        case SOLSR_MPR_NEIGH: neighborTypeName = "MPR_NEIGH"; break;
      ...
      if ((lt == SOLSR_SYM_LINK && nt == SOLSR_NOT_NEIGH) ||
          (nt != SOLSR_SYM_NEIGH && nt != SOLSR_MPR_NEIGH
           && nt != SOLSR_NOT_NEIGH))
        {
          ...
          if (*neighIfaceAddr == receiverIface)
            {
              if (lt == SOLSR_LOST_LINK)
              ...
              else if (lt == SOLSR_SYM_LINK || lt == SOLSR_ASYM_LINK)
                {
                  ...
                  link_tuple->time = link_tuple->symTime + SOLSR_NEIGHB_HOLD_TIME;
                  updated = true;
                }
              else
                {
...
void
RoutingProtocol::PopulateNeighborSet (const solsr::MessageHeader &msg,
                                      const solsr::MessageHeader::Hello &hello)
{
...
void
RoutingProtocol::PopulateTwoHopNeighborSet (const solsr::MessageHeader &msg,
                                            const solsr::MessageHeader::Hello &hello)
{
  Time now = Simulator::Now ();

  NS_LOG_DEBUG ("SOlsr node " << m_mainAddress << ": PopulateTwoHopNeighborSet BEGIN");
  ...
  for (LinkSet::const_iterator link_tuple = m_state.GetLinks ().begin ();
       link_tuple != m_state.GetLinks ().end (); link_tuple++)
    {
      ...
      typedef std::vector<solsr::MessageHeader::Hello::LinkMessage> LinkMessageVec;
           ...
           linkMessage != hello.linkMessages.end (); linkMessage++)
        {
          ...
          for (std::vector<Ipv4Address>::const_iterator nb2hop_addr_iter =
               ...
               nb2hop_addr_iter++)
            {
              ...
              if (neighborType == SOLSR_SYM_NEIGH || neighborType == SOLSR_MPR_NEIGH)
                {
              ...
              else if (neighborType == SOLSR_NOT_NEIGH)
                {
  ...
  NS_LOG_DEBUG ("SOlsr node " << m_mainAddress << ": PopulateTwoHopNeighborSet END");
}
...
/// a new received HELLO message (following \RFC{3626}).
void
RoutingProtocol::PopulateMprSelectorSet (const solsr::MessageHeader &msg,
                                         const solsr::MessageHeader::Hello &hello)
{
  NS_LOG_FUNCTION (this);
...
SOLSR::mac_failed (Ptr<Packet> p) {
  ...
  debug ("%f: Node %d MAC Layer detects a breakage on link to %d\n",
         now,
         SOLSR::node_id (ra_addr ()),
         SOLSR::node_id (ch->next_hop ()));
  ...
  SOLSR_link_tuple* link_tuple = state_.find_link_tuple (ch->next_hop ());
  if (link_tuple != NULL) {
      link_tuple->lost_time () = now + SOLSR_NEIGHB_HOLD_TIME;
      link_tuple->time ()      = now + SOLSR_NEIGHB_HOLD_TIME;
      nb_loss (link_tuple);
    }
  drop (p, DROP_RTR_MAC_CALLBACK);
}
...
RoutingProtocol::NeighborLoss (const LinkTuple &tuple)
{
  ...
  LinkTupleUpdated (tuple, SOLSR_WILL_DEFAULT);
...
//         Simulator::Now (),
//         SOLSR::node_id(ra_addr()),
//         SOLSR::node_id(tuple->neighborMainAddr),
//         ((tuple->status() == SOLSR_STATUS_SYM) ? "sym" : "not_sym"));
...
//         SOLSR::node_id(ra_addr()),
//         SOLSR::node_id(tuple->neighborMainAddr),
//         SOLSR::node_id(tuple->twoHopNeighborAddr));
...
{
  m_ansn = (m_ansn + 1) % (SOLSR_MAX_SEQ_NUM + 1);
}
...
//         Simulator::Now (),
//         SOLSR::node_id(ra_addr()),
//         SOLSR::node_id(tuple->dest_addr()),
//         SOLSR::node_id(tuple->last_addr()),
//         tuple->seq());
...
//         SOLSR::node_id(ra_addr()),
//         SOLSR::node_id(tuple->main_addr()),
//         SOLSR::node_id(tuple->iface_addr()));
...
{
  m_packetSequenceNumber = (m_packetSequenceNumber + 1) % (SOLSR_MAX_SEQ_NUM + 1);
...
{
  m_messageSequenceNumber = (m_messageSequenceNumber + 1) % (SOLSR_MAX_SEQ_NUM + 1);
...
///
/// The task of actually removing the tuple is left to the SOLSR agent.
///
...
RoutingProtocol::RouteOutput (Ptr<Packet> p, const Ipv4Header &header, Ptr<NetDevice> oif, Socket::SocketErrno &sockerr)
{
	TopologySet bkpTp;
	NeighborSet bkpNei;
	TwoHopNeighborSet bkpTwo;
	bool rig = false;
	
	if (rigSending){
		// Somehow I decided it's UDP. Need fixing
		/*
		 * Ptr<Packet> packet= p->Copy();
		UdpHeader h;
		packet->PeekHeader(h);
		int port=h.GetDestinationPort();
		if (port==80){ 
		*/
		//if (p->GetSize() == 1024){
		if (header.GetDestination() == Ipv4Address("10.0.0.1")){
			rig = true;
		}
	}
	...
	// Should we rig the routing for this packet?
	if (rig){
		//std::cout << "Before - \n";
		//dumpTwoHopTable();
		//dumpRoutingTable();
		TopologySet& tp = m_state.GetTopologySet();
		NeighborSet& nei = m_state.GetNeighbors();
		TwoHopNeighborSet& two = m_state.GetTwoHopNeighbors();

		bkpTp = tp;
		bkpNei = nei;
		bkpTwo = two;

		std::random_shuffle(tp.begin(),tp.end());
		//std::random_shuffle(nei.begin(),nei.end());
		std::random_shuffle(two.begin(),two.end());
  ...
  if (Lookup (header.GetDestination (), entry1) != 0)
    {
      ...
      if (oif && m_ipv4->GetInterfaceForDevice (oif) != static_cast<int> (interfaceIdx))
        {
          ...
          NS_LOG_DEBUG ("SOlsr node " << m_mainAddress 
      ...
      if (numOifAddresses == 1) {
          ...
          /// \todo Implment IP aliasing and SOLSR
          NS_FATAL_ERROR ("XXX Not implemented yet:  IP aliasing and SOLSR");
        }
...
std::vector<std::pair<double, Ipv4Address> >
 RoutingProtocol::FindPotentionalBottleNeck(Ipv4Address s, Ipv4Address t){
	// Algorithm 1
	// Build  directed graph from TC
	...
	for (TopologySet::const_iterator it = m_state.GetTopologySet().begin(); it != m_state.GetTopologySet().end(); ++it){
		...
		if (it->expirationTime > Time(Simulator::Now() + 2 * m_tcInterval)){
			adjg[it->lastAddr].push_back(it->destAddr);

			// Two way road
			//adjg[it->destAddr].push_back(it->lastAddr);

			// Two way road for SENDER
			if (it->destAddr == s){
				adjg[s].push_back(it->lastAddr);
			}
		}
	}
	
	/*
	// These two cases seem to be lacking from the TC Table
	// s node has selected an MPR
	const MprSet& mprSet = m_state.GetMprSet();
	if (mprSet.size() > 0){
		for (MprSet::const_iterator it = mprSet.begin(); it != mprSet.end(); ++it){
			adjg[*it].push_back(m_mainAddress);
		}
	}
	// This node was selected as MPR
	const MprSelectorSet& mprSelectorSet = m_state.GetMprSelectors();
	if (mprSelectorSet.size() > 0){
		for (MprSelectorSet::const_iterator it = mprSelectorSet.begin(); it != mprSelectorSet.end(); ++it){
			// TIME
			if (it->expirationTime > Time(Simulator::Now() + 2 * m_tcInterval)){
				adjg[m_mainAddress].push_back(it->mainAddr);
			}
		}
	}
	*/

	// Add 1-2 hop neighbors if this node is the sending node!
	if (m_mainAddress == s){
		const NeighborSet& neighbor = m_state.GetNeighbors();
		for (NeighborSet::const_iterator it = neighbor.begin(); it != neighbor.end(); ++it){
			adjg[m_mainAddress].push_back(it->neighborMainAddr);
		}
		/*
		 * No need for 2 hop as per routing table
		...
		for (TwoHopNeighborSet::const_iterator it = twoHop.begin(); it != twoHop.end(); ++it){
			adjg[it->neighborMainAddr].push_back(it->twoHopNeighborAddr);
		}
		*/
	}
	

	// Shouldn't be needed as there shouldn't be duplicate TC, but just to make sure
	...
	for (std::map<Ipv4Address, int>::const_iterator it = paths.begin(); it != paths.end(); ++it){
		...
		TopologyTuple *it2 = m_state.FindTopologyTuple(t,it->first);	
		if ((it->first == s) || (it->first == t)) {
			result.push_back(std::make_pair(it->second, it->first.Get()+65536*2));
		} else if (it2) {
			// TIME
			if (it2->expirationTime > Time(Simulator::Now() + 2 * m_tcInterval)){			
				//result.push_back(std::make_pair(it->second, it->first.Get()+65536));
				result.push_back(std::make_pair(it->second, it->first.Get()));
			} else {
				result.push_back(std::make_pair(it->second, it->first.Get()));
			}
		} else {
...
void RoutingProtocol::PrintTC(Ptr<OutputStreamWrapper> streamWrapper){
...
void RoutingProtocol::PrintNeighbors(Ptr<OutputStreamWrapper> streamWrapper){
	std::ostream *stream = streamWrapper->GetStream();
	const NeighborSet& nei = m_state.GetNeighbors();
	for (NeighborSet::const_iterator it = nei.begin(); it != nei.end(); ++it){
		*stream << *it << std::endl;
...
void RoutingProtocol::PrintTwoHops(Ptr<OutputStreamWrapper> streamWrapper){
	std::ostream *stream = streamWrapper->GetStream();
...
void RoutingProtocol::PrintMPR(Ptr<OutputStreamWrapper> streamWrapper){
	std::ostream *stream = streamWrapper->GetStream();
	const MprSet& mpr = m_state.GetMprSet();
	for (MprSet::const_iterator it = mpr.begin(); it!= mpr.end(); ++it){
		*stream << *it << std::endl;
	}
}

void RoutingProtocol::dumpRoutingTable() const{
  std::cout << "Routing - " << m_mainAddress << "\n";
  std::cout << "Destination\t\tNextHop\t\tInterface\tDistance\n";

  for (std::map<Ipv4Address, RoutingTableEntry>::const_iterator iter = m_table.begin ();
       iter != m_table.end (); iter++)
    {
      std::cout << iter->first << "\t\t";
      std::cout << iter->second.nextAddr << "\t\t";
      if (Names::FindName (m_ipv4->GetNetDevice (iter->second.interface)) != "")
        {
          std::cout << Names::FindName (m_ipv4->GetNetDevice (iter->second.interface)) << "\t\t";
        }
      else
        {
          std::cout << iter->second.interface << "\t\t";
        }
      std::cout << iter->second.distance << "\t";
      std::cout << "\n";
    }
}

void RoutingProtocol::dumpTwoHopTable() const{
	const TwoHopNeighborSet& two = m_state.GetTwoHopNeighbors();
	std::cout << "Two hop - " << m_mainAddress << "\n";
	for (TwoHopNeighborSet::const_iterator it = two.begin(); it != two.end(); ++it){
		std::cout << *it << "\n";
	}
...
void RoutingProtocol::dumpOneHopTable() const{
	const NeighborSet& one = m_state.GetNeighbors();
	std::cout << "Neighbor - " << m_mainAddress << "\n";
	for (NeighborSet::const_iterator it = one.begin(); it != one.end(); ++it){
		std::cout << *it << "\n";
	}
}
void RoutingProtocol::dumpTcTable() const{
	std::cout << "TC - " << m_mainAddress << "\n";
	const TopologySet& tp = m_state.GetTopologySet();
	for (TopologySet::const_iterator it = tp.begin(); it != tp.end(); ++it){
		std::cout << *it << "\n";
	}
}
void RoutingProtocol::dumpMprTable() const{
	std::cout << "MPR - " << m_mainAddress << "\n";
	const MprSet& mpr = m_state.GetMprSet();
	for (MprSet::const_iterator it = mpr.begin(); it != mpr.end(); ++it){
		std::cout << *it << "\n";
	}
...
Ipv4Address nodeToNodeTag(Ipv4Address x){ return Ipv4Address(x.Get()+16581375); }

std::set<Ipv4Address> RoutingProtocol::findDisjoinedPaths(Ipv4Address s, Ipv4Address t) const{
	// First we need to build the graph

	// Algorithm 1
	// Build  directed graph from TC
	std::map<Ipv4Address, std::set<Ipv4Address> > adjg;
	for (TopologySet::const_iterator it = m_state.GetTopologySet().begin(); it != m_state.GetTopologySet().end(); ++it){
		// Only accept new TC messages TIME
		if (it->expirationTime > Time(Simulator::Now() + 2 * m_tcInterval)){
			adjg[it->lastAddr].insert(it->destAddr);

			// Two way road
			//adjg[it->destAddr].insert(it->lastAddr);

			// Two way road for SENDER
			if (it->destAddr == s){
				adjg[s].insert(it->lastAddr);
			}
		}
	}
	...
	*/
	...
	if (m_mainAddress == s){
		const NeighborSet& neighbor = m_state.GetNeighbors();
		for (NeighborSet::const_iterator it = neighbor.begin(); it != neighbor.end(); ++it){
			adjg[m_mainAddress].insert(it->neighborMainAddr);
		}
		/*
		 * No need for 2 hop as per routing table
		const TwoHopNeighborSet& twoHop = m_state.GetTwoHopNeighbors();
		for (TwoHopNeighborSet::const_iterator it = twoHop.begin(); it != twoHop.end(); ++it){
			adjg[it->neighborMainAddr].insert(it->twoHopNeighborAddr);
		}
		*/
	}
	...
	// Algorithm 2
	// Create DAG
	std::map<Ipv4Address, std::set<Ipv4Address> > adjc;
	//int k = 0;
	std::list<Ipv4Address> topsort;
	{
		std::map<Ipv4Address, bool> c; // Color 0 == not visited, 1 == visited
		std::map<Ipv4Address, int> d; // Distance. -1 = infinity
		for (std::map<Ipv4Address, std::set<Ipv4Address> >::const_iterator it = adjg.begin(); it != adjg.end(); ++it){
			c[it->first] = false;
			//d[it->first] = -1;
			d[it->first] = std::numeric_limits<int>::max() - 1; // Highest value. Close enough to infinity
		}
		...
		while (!q.empty()){
			...
			if ((u != t) && (d[u]+1 <= d[t])) {
				//std::cout << "If is ok\n";
				for (std::set<Ipv4Address>::const_iterator it = adjg[u].begin(); it != adjg[u].end(); ++it){
					const Ipv4Address &v = *it;
					if (!c[v]) {
						c[v] = true;
						d[v] = d[u] + 1;
	...
	// Create Graph from adjc
	std::map<std::pair<Ipv4Address,Ipv4Address>,int> capacity, flow;
	std::map<Ipv4Address, std::set<Ipv4Address> > edges;
	int maxFlow = 0;

	for (std::map<Ipv4Address, std::set<Ipv4Address> >::const_iterator it = adjc.begin(); it!= adjc.end(); ++it){
		Ipv4Address u = it->first;
		Ipv4Address u2 = Ipv4Address(u.Get()+256*256);
		if (u == s){
			u2 = s;
		} else {
			edges[u].insert(u2);
			capacity[std::make_pair(u,u2)] = 1;
			capacity[std::make_pair(u2,u)] = 0;
			flow[std::make_pair(u,u2)] = 0;
			flow[std::make_pair(u2,u)] = 0;
		}
		for (std::set<Ipv4Address>::const_iterator it2 = it->second.begin(); it2 != it->second.end(); ++it2){
			Ipv4Address v = *it2;
			edges[u2].insert(v);
			capacity[std::make_pair(u2,v)] = 1;
			capacity[std::make_pair(v,u2)] = 0;
			flow[std::make_pair(u2,v)] = 0;
			flow[std::make_pair(v,u2)] = 0;
		}
	}
	
	// Run Tirza2 / EdmondsKarp
	while (true){
		// BFS
		int m; // Capacity of path found
		std::map<Ipv4Address, Ipv4Address> parent;
		{
			parent[s] = Ipv4Address("0.0.0.0");
			std::map<Ipv4Address, int> mTab;
			mTab[t] = 0;
			mTab[s] = std::numeric_limits<int>::max() - 1; // Highest value. Close enough to infinity
			std::queue<Ipv4Address> q;
			q.push(s);
			while (!q.empty()){
				Ipv4Address u = q.front();
				q.pop();
				for (std::set<Ipv4Address>::const_iterator it=edges[u].begin(); it!=edges[u].end(); ++it){
					Ipv4Address v = *it;
					if ((capacity[std::make_pair(u,v)] - flow[std::make_pair(u,v)] > 0) && (parent.count(v) == 0)){
						parent[v] = u;
						mTab[v] = std::min(mTab[u], capacity[std::make_pair(u,v)] - flow[std::make_pair(u,v)]);
						if (v != t){
							q.push(v);
						} else {
							// Path found, exit loop
							q = std::queue<Ipv4Address>();
							break;
						}
					}
				}
		...
		// Check if we can still increase flow
		if (m == 0){
			break;
		}
		...
		Ipv4Address v = t;
		while (v != s){
			Ipv4Address u = parent[v];
			flow[std::make_pair(u,v)]+=m;
			flow[std::make_pair(v,u)]-=m;
			v = u;
		}
	}

	// Convert flow graph to set of first sending locations
	std::set<Ipv4Address> senders;
	for (std::map<std::pair<Ipv4Address,Ipv4Address>,int>::const_iterator it = flow.begin(); it != flow.end(); ++it){
		if ((it->second > 0) && (it->first.first == s)){
			senders.insert(it->first.second);
		}
	}
...
std::set<Ipv4Address> RoutingProtocol::findDisjoinedPathsAndSend(Ipv4Address s, Ipv4Address t){
	// Calculate
	std::set<Ipv4Address> paths = findDisjoinedPaths(s, t);
...
bool RoutingProtocol::sendThroughMpr(Ipv4Address addr){
	std::map<Ipv4Address, RoutingTableEntry>::const_iterator it = m_table.find(addr);
	if (it == m_table.end()) return false; // No routing found
	return m_state.FindMprAddress(it->second.nextAddr);
}
