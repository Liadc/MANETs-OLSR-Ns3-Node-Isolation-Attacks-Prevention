...
const char * const HelloRegressionTest::PREFIX = "solsr-hello-regression-test";

HelloRegressionTest::HelloRegressionTest() : 
  TestCase ("Test SOLSR Hello messages generation"),
  m_time (Seconds (5))
{
}
...
HelloRegressionTest::CreateNodes ()
{
  ...
  NodeContainer c;
  c.Create (2);
  // install TCP/IP & SOLSR
  SOlsrHelper solsr;
  InternetStackHelper internet;
  internet.SetRoutingHelper (solsr);
  internet.Install (c);
  // Assign SOLSR RVs to specific streams
  int64_t streamsUsed = solsr.AssignStreams (c, 0);
