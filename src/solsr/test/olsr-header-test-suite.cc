...
#include "ns3/test.h"
#include "ns3/solsr-header.h"
#include "ns3/packet.h"

namespace ns3 {

class SOlsrEmfTestCase : public TestCase {
public:
  SOlsrEmfTestCase ();
  virtual void DoRun (void);
};

SOlsrEmfTestCase::SOlsrEmfTestCase ()
  : TestCase ("Check Emf solsr time conversion")
{
}
void
SOlsrEmfTestCase::DoRun (void)
{
  for (int time = 1; time <= 30; time++)
    {
      uint8_t emf = solsr::SecondsToEmf (time);
      double seconds = solsr::EmfToSeconds (emf);
...
class SOlsrMidTestCase : public TestCase {
public:
  SOlsrMidTestCase ();
  virtual void DoRun (void);
};

SOlsrMidTestCase::SOlsrMidTestCase ()
  : TestCase ("Check Mid solsr messages")
{
}
void
SOlsrMidTestCase::DoRun (void)
{
  Packet packet;

  {
    solsr::PacketHeader hdr;
    solsr::MessageHeader msg1;
    solsr::MessageHeader::Mid &mid1 = msg1.GetMid ();
    solsr::MessageHeader msg2;
    solsr::MessageHeader::Mid &mid2 = msg2.GetMid ();
    ...
    msg2.SetMessageType (solsr::MessageHeader::MID_MESSAGE);
    msg2.SetMessageSequenceNumber (7);

    // Build an SOLSR packet header
...

  {
    ...
    uint32_t sizeLeft = hdr.GetPacketLength () - hdr.GetSerializedSize ();
    {
      ...
      NS_TEST_ASSERT_MSG_EQ (msg1.GetMessageType (), solsr::MessageHeader::MID_MESSAGE, "204");
    ...
    }
    {
      ...
      NS_TEST_ASSERT_MSG_EQ (msg2.GetMessageType (), solsr::MessageHeader::MID_MESSAGE, "212");
...
class SOlsrHelloTestCase : public TestCase {
public:
  SOlsrHelloTestCase ();
  virtual void DoRun (void);
};

SOlsrHelloTestCase::SOlsrHelloTestCase ()
  : TestCase ("Check Hello solsr messages")
{
}
void
SOlsrHelloTestCase::DoRun (void)
{
  Packet packet;
  solsr::MessageHeader msgIn;
  solsr::MessageHeader::Hello &helloIn = msgIn.GetHello ();

  helloIn.SetHTime (Seconds (7));
  helloIn.willingness = 66;

  {
    solsr::MessageHeader::Hello::LinkMessage lm1;
    lm1.linkCode = 2;
    ...
    solsr::MessageHeader::Hello::LinkMessage lm2;
    lm2.linkCode = 3;
  ...
  solsr::MessageHeader msgOut;
  packet.RemoveHeader (msgOut);
  solsr::MessageHeader::Hello &helloOut = msgOut.GetHello ();
...
class SOlsrTcTestCase : public TestCase {
public:
  SOlsrTcTestCase ();
  virtual void DoRun (void);
};

SOlsrTcTestCase::SOlsrTcTestCase ()
  : TestCase ("Check Tc solsr messages")
{
}
void
SOlsrTcTestCase::DoRun (void)
{
  Packet packet;
  solsr::MessageHeader msgIn;
  solsr::MessageHeader::Tc &tcIn = msgIn.GetTc ();
  ...
  solsr::MessageHeader msgOut;
  packet.RemoveHeader (msgOut);
  solsr::MessageHeader::Tc &tcOut = msgOut.GetTc ();
...
class SOlsrHnaTestCase : public TestCase {
public:
  SOlsrHnaTestCase ();
  virtual void DoRun (void);
};

SOlsrHnaTestCase::SOlsrHnaTestCase ()
  : TestCase ("Check Hna solsr messages")
{
}

void
SOlsrHnaTestCase::DoRun (void)
{
  Packet packet;
  solsr::MessageHeader msgIn;
  solsr::MessageHeader::Hna &hnaIn = msgIn.GetHna ();

  hnaIn.associations.push_back ((solsr::MessageHeader::Hna::Association)
  ...
  solsr::MessageHeader msgOut;
  packet.RemoveHeader (msgOut);
  solsr::MessageHeader::Hna &hnaOut = msgOut.GetHna ();
...
static class SOlsrTestSuite : public TestSuite
{
public:
  SOlsrTestSuite ();
} g_solsrTestSuite;

SOlsrTestSuite::SOlsrTestSuite()
  : TestSuite ("routing-solsr-header", UNIT)
{
  AddTestCase (new SOlsrHnaTestCase (), TestCase::QUICK);
  AddTestCase (new SOlsrTcTestCase (), TestCase::QUICK);
  AddTestCase (new SOlsrHelloTestCase (), TestCase::QUICK);
  AddTestCase (new SOlsrMidTestCase (), TestCase::QUICK);
  AddTestCase (new SOlsrEmfTestCase (), TestCase::QUICK);
}
