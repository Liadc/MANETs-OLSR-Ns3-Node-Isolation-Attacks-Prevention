...
//
// One way is to use iolsr::RoutingProtocol::SetRoutingTableAssociation ()
// to use which you may run:
//
// ./waf --run "iolsr-hna --assocMethod1=1"
//
// The other way is to use iolsr::RoutingProtocol::AddHostNetworkAssociation ()
// to use which you may run:
//
// ./waf --run "iolsr-hna --assocMethod2=1"
//
...
NS_LOG_COMPONENT_DEFINE ("IOlsrHna");
...
int main (int argc, char *argv[])
{
  ...
  NetDeviceContainer devices = wifi.Install (wifiPhy, wifiMac, iolsrNodes);

  CsmaHelper csma;
  csma.SetChannelAttribute ("DataRate", DataRateValue (DataRate (5000000)));
  csma.SetChannelAttribute ("Delay", TimeValue (MilliSeconds (2)));
  NetDeviceContainer csmaDevices = csma.Install (NodeContainer (csmaNodes.Get (0), iolsrNodes.Get (1)));
  ...
  mobility.Install (iolsrNodes);

  IOlsrHelper iolsr;

  // Specify Node B's csma device as a non-IOLSR device.
  iolsr.ExcludeInterface (iolsrNodes.Get (1), 2);
  ...
  InternetStackHelper internet_iolsr;
  internet_iolsr.SetRoutingHelper (list); // has effect on the next Install ()
  internet_iolsr.Install (iolsrNodes);
  ...
  Ptr<Socket> source = Socket::CreateSocket (iolsrNodes.Get (0), tid);
  InetSocketAddress remote = InetSocketAddress (Ipv4Address ("172.16.1.1"), 80);
  source->Connect (remote);

  // Obtain iolsr::RoutingProtocol instance of gateway node
  // (namely, node B) and add the required association
  Ptr<Ipv4> stack = iolsrNodes.Get (1)->GetObject<Ipv4> ();
  Ptr<Ipv4RoutingProtocol> rp_Gw = (stack->GetRoutingProtocol ());
  Ptr<Ipv4ListRouting> lrp_Gw = DynamicCast<Ipv4ListRouting> (rp_Gw);

  Ptr<iolsr::RoutingProtocol> iolsrrp_Gw;

  for (uint32_t i = 0; i < lrp_Gw->GetNRoutingProtocols ();  i++)
    {
      int16_t priority;
      Ptr<Ipv4RoutingProtocol> temp = lrp_Gw->GetRoutingProtocol (i, priority);
      if (DynamicCast<iolsr::RoutingProtocol> (temp))
        {
          iolsrrp_Gw = DynamicCast<iolsr::RoutingProtocol> (temp);
        }
    }
  ...
  if (assocMethod1)
    {
      ...
      // which are associated with non-IOLSR interfaces specified above.
      hnaEntries->AddNetworkRouteTo (Ipv4Address ("172.16.1.0"), Ipv4Mask ("255.255.255.0"), uint32_t (2), uint32_t (1));
      iolsrrp_Gw->SetRoutingTableAssociation (hnaEntries);
    }
 
  if (assocMethod2)
    {
      // Specify the required associations directly.
      iolsrrp_Gw->AddHostNetworkAssociation (Ipv4Address ("172.16.1.0"), Ipv4Mask ("255.255.255.0"));
    }

  // Tracing
  wifiPhy.EnablePcap ("iolsr-hna", devices);
  csma.EnablePcap ("iolsr-hna", csmaDevices, false);
