...
const char * const HelloRegressionTest::PREFIX = "iolsr-hello-regression-test";

HelloRegressionTest::HelloRegressionTest() : 
  TestCase ("Test IOLSR Hello messages generation"),
  m_time (Seconds (5))
{
}
...
HelloRegressionTest::CreateNodes ()
{
  ...
  NodeContainer c;
  c.Create (2);
  // install TCP/IP & IOLSR
  IOlsrHelper iolsr;
  InternetStackHelper internet;
  internet.SetRoutingHelper (iolsr);
  internet.Install (c);
  // Assign IOLSR RVs to specific streams
  int64_t streamsUsed = iolsr.AssignStreams (c, 0);
