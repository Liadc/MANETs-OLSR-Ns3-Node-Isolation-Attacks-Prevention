...
const char * const HelloRegressionTest::PREFIX = "ibolsr-hello-regression-test";

HelloRegressionTest::HelloRegressionTest() : 
  TestCase ("Test IBOLSR Hello messages generation"),
  m_time (Seconds (5))
{
}
...
HelloRegressionTest::CreateNodes ()
{
  ...
  NodeContainer c;
  c.Create (2);
  // install TCP/IP & IBOLSR
  IBOlsrHelper ibolsr;
  InternetStackHelper internet;
  internet.SetRoutingHelper (ibolsr);
  internet.Install (c);
  // Assign IBOLSR RVs to specific streams
  int64_t streamsUsed = ibolsr.AssignStreams (c, 0);
