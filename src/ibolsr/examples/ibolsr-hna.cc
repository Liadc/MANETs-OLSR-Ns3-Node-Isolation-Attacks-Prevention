...
//
// One way is to use ibolsr::RoutingProtocol::SetRoutingTableAssociation ()
// to use which you may run:
//
// ./waf --run "ibolsr-hna --assocMethod1=1"
//
// The other way is to use ibolsr::RoutingProtocol::AddHostNetworkAssociation ()
// to use which you may run:
//
// ./waf --run "ibolsr-hna --assocMethod2=1"
//
...
NS_LOG_COMPONENT_DEFINE ("IBOlsrHna");
...
int main (int argc, char *argv[])
{
  ...
  NodeContainer ibolsrNodes;
  ibolsrNodes.Create (2);
  ...
  NetDeviceContainer devices = wifi.Install (wifiPhy, wifiMac, ibolsrNodes);

  CsmaHelper csma;
  csma.SetChannelAttribute ("DataRate", DataRateValue (DataRate (5000000)));
  csma.SetChannelAttribute ("Delay", TimeValue (MilliSeconds (2)));
  NetDeviceContainer csmaDevices = csma.Install (NodeContainer (csmaNodes.Get (0), ibolsrNodes.Get (1)));
  ...
  mobility.Install (ibolsrNodes);

  IBOlsrHelper ibolsr;

  // Specify Node B's csma device as a non-IBOLSR device.
  ibolsr.ExcludeInterface (ibolsrNodes.Get (1), 2);
  ...
  InternetStackHelper internet_ibolsr;
  internet_ibolsr.SetRoutingHelper (list); // has effect on the next Install ()
  internet_ibolsr.Install (ibolsrNodes);
  ...
  Ptr<Socket> source = Socket::CreateSocket (ibolsrNodes.Get (0), tid);
  InetSocketAddress remote = InetSocketAddress (Ipv4Address ("172.16.1.1"), 80);
  source->Connect (remote);

  // Obtain ibolsr::RoutingProtocol instance of gateway node
  // (namely, node B) and add the required association
  Ptr<Ipv4> stack = ibolsrNodes.Get (1)->GetObject<Ipv4> ();
  Ptr<Ipv4RoutingProtocol> rp_Gw = (stack->GetRoutingProtocol ());
  Ptr<Ipv4ListRouting> lrp_Gw = DynamicCast<Ipv4ListRouting> (rp_Gw);

  Ptr<ibolsr::RoutingProtocol> ibolsrrp_Gw;

  for (uint32_t i = 0; i < lrp_Gw->GetNRoutingProtocols ();  i++)
    {
      int16_t priority;
      Ptr<Ipv4RoutingProtocol> temp = lrp_Gw->GetRoutingProtocol (i, priority);
      if (DynamicCast<ibolsr::RoutingProtocol> (temp))
        {
          ibolsrrp_Gw = DynamicCast<ibolsr::RoutingProtocol> (temp);
        }
    }
  ...
  if (assocMethod1)
    {
      ...
      // which are associated with non-IBOLSR interfaces specified above.
      hnaEntries->AddNetworkRouteTo (Ipv4Address ("172.16.1.0"), Ipv4Mask ("255.255.255.0"), uint32_t (2), uint32_t (1));
      ibolsrrp_Gw->SetRoutingTableAssociation (hnaEntries);
    }
 
  if (assocMethod2)
    {
      // Specify the required associations directly.
      ibolsrrp_Gw->AddHostNetworkAssociation (Ipv4Address ("172.16.1.0"), Ipv4Mask ("255.255.255.0"));
    }

  // Tracing
  wifiPhy.EnablePcap ("ibolsr-hna", devices);
  csma.EnablePcap ("ibolsr-hna", csmaDevices, false);
