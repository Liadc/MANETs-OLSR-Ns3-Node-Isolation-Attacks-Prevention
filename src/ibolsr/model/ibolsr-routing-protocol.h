...
#ifndef IBOLSR_AGENT_IMPL_H
#define IBOLSR_AGENT_IMPL_H

#include "ibolsr-header.h"
#include "ns3/test.h"
#include "ibolsr-state.h"
#include "ibolsr-repositories.h"
...
///
/// \defgroup ibolsr IBOLSR Routing
/// This section documents the API of the ns-3 IBOLSR module. For a generic 
/// functional description, please refer to the ns-3 manual.

/// An %IBOLSR's routing table entry.
...
class IBOlsrMprTestCase;

///
/// \ingroup ibolsr
///
/// \brief IBOLSR routing protocol for IPv4
///
class RoutingProtocol : public Ipv4RoutingProtocol
{
public:
  friend class IBOlsrMprTestCase;
  ...
  bool blackholeAttack;
  Ipv4Address blackholeAttackTarget;
  virtual Ipv4Address ExecuteBlackholeAttack();
  virtual Ipv4Address ExecuteBlackholeAttack(Ipv4Address);
  ...
  bool activeDefence;
  bool activeImpDefence;
  void activateFictiveDefence();
  void activateImpDefence();
  ...
  // Colluding attack
  bool blackholeAttackColl1;
  bool blackholeAttackColl2;
  Ipv4Address blackholeAttackCollFriend;
  // Get target and friend
  void ExecuteBlackholeAttackColl1(Ipv4Address, Ipv4Address);
  void ExecuteBlackholeAttackColl2(Ipv4Address, Ipv4Address);
  ...
  ///
  /// \brief Set the IBOLSR main address to the first address on the indicated
  ///        interface
  ...
  /**
   * Return the list of routing table entries discovered by IBOLSR
   **/
...
private:
  ...
  void RecvIBOlsr (Ptr<Socket> socket);

  void MprComputation ();
  void RoutingTableComputation ();
  Ipv4Address GetMainAddress (Ipv4Address iface_addr) const;
  bool UsesNonIBOlsrOutgoingInterface (const Ipv4RoutingTableEntry &route);
  ...
  ibolsr::MessageList m_queuedMessages;
  Timer m_queuedMessagesTimer; // timer for throttling outgoing messages

  void ForwardDefault (ibolsr::MessageHeader ibolsrMessage,
                       DuplicateTuple *duplicated,
                       const Ipv4Address &localIface,
                       const Ipv4Address &senderAddress);
  void QueueMessage (const ibolsr::MessageHeader &message, Time delay);
  ...
  void ProcessHello (const ibolsr::MessageHeader &msg,
                     const Ipv4Address &receiverIface,
                     const Ipv4Address &senderIface);
  void ProcessTc (const ibolsr::MessageHeader &msg,
                  const Ipv4Address &senderIface);
  void ProcessMid (const ibolsr::MessageHeader &msg,
                   const Ipv4Address &senderIface);
  void ProcessHna (const ibolsr::MessageHeader &msg,
                   const Ipv4Address &senderIface);

  void LinkSensing (const ibolsr::MessageHeader &msg,
                    const ibolsr::MessageHeader::Hello &hello,
                    const Ipv4Address &receiverIface,
                    const Ipv4Address &sender_iface);
  void PopulateNeighborSet (const ibolsr::MessageHeader &msg,
                            const ibolsr::MessageHeader::Hello &hello);
  void PopulateTwoHopNeighborSet (const ibolsr::MessageHeader &msg,
                                  const ibolsr::MessageHeader::Hello &hello);
  void PopulateMprSelectorSet (const ibolsr::MessageHeader &msg,
                               const ibolsr::MessageHeader::Hello &hello);
  ...
  // (reason: for IBOLSR Link Sensing we need to know on which interface
