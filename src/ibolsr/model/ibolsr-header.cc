...
#define IBOLSR_MSG_HEADER_SIZE 12
#define IBOLSR_PKT_HEADER_SIZE 4

namespace ns3 {
namespace ibolsr {


NS_LOG_COMPONENT_DEFINE ("IBOlsrHeader");

/// Scaling factor used in RFC 3626.
#define IBOLSR_C 0.0625
...
SecondsToEmf (double seconds)
{
  ...
  for (b = 0; (seconds/IBOLSR_C) >= (1 << b); ++b)
    ;
  NS_ASSERT ((seconds/IBOLSR_C) < (1 << b));
  b--;
  NS_ASSERT ((seconds/IBOLSR_C) >= (1 << b));

  // compute the expression 16*(T/(C*(2^b))-1), which may not be a integer
  double tmp = 16*(seconds/(IBOLSR_C*(1<<b))-1);
...
///
/// \param ibolsr_format number of seconds in mantissa/exponent format.
/// \return the decimal number of seconds.
///
double
EmfToSeconds (uint8_t ibolsrFormat)
{
  int a = (ibolsrFormat >> 4);
  int b = (ibolsrFormat & 0xf);
  // value = C*(1+a/16)*2^b [in seconds]
  return IBOLSR_C * (1 + a/16.0) * (1 << b);
}
...
{
  static TypeId tid = TypeId ("ns3::ibolsr::PacketHeader")
    .SetParent<Header> ()
...
{
  static TypeId tid = TypeId ("ns3::ibolsr::MessageHeader")
    .SetParent<Header> ()
...
{
  uint32_t size = IBOLSR_MSG_HEADER_SIZE;
...
MessageHeader::Deserialize (Buffer::Iterator start)
{
  ...
  size = IBOLSR_MSG_HEADER_SIZE;
  switch (m_messageType)
    {
    case MID_MESSAGE:
      size += m_message.mid.Deserialize (i, m_messageSize - IBOLSR_MSG_HEADER_SIZE);
      break;
    case HELLO_MESSAGE:
      size += m_message.hello.Deserialize (i, m_messageSize - IBOLSR_MSG_HEADER_SIZE);
      break;
    case TC_MESSAGE:
      size += m_message.tc.Deserialize (i, m_messageSize - IBOLSR_MSG_HEADER_SIZE);
      break;
    case HNA_MESSAGE:
      size += m_message.hna.Deserialize (i, m_messageSize - IBOLSR_MSG_HEADER_SIZE);
      break;
    default:
      NS_ASSERT (false);
    }
